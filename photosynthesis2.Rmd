...
#head(photosynthesisDS)
photosynthesisDS <- binaryDS %>%
  mutate(Gsp = paste(Updated_Genus, Updated_species, sep = "_")) %>% 
  filter(!is.na(Binary_reduced_PR_ratio) | !is.na(Binary_reduced_FvFm))
photosynthesisDS %>% tally() #n=438 datapoints, tally() returns same result as count(photosynthesisDS)
photosynthesisDS %>% count(Ref, sort=T) #n=15 studies
photosynthesisDS %>% count(Ref_name, sort=T) #n=15 studies, count(Ref_name) = group_by(Ref_name) %>% tally()
photosynthesisDS %>% count(Ocean, sort=T) #n=4 oceans (one is Indo-Pacific)
photosynthesisDS %>% count(Gsp) %>% #n=36 species
  top_n(10)
photosynthesisDS %>% count(Updated_Genus, sort=T) %>% #n=23 genera
  top_n(10)
```
...
photoSS <- binarySS %>%
  ...
  mutate(Sed_level_mg_L_num = as.numeric(Sed_level_mg_L))
...
photo_eff <- photosynthesisDS %>% 
  select(Ref, Ref_name, Control_code, Sed_level_mg_cm2_d, Binary_reduced_FvFm, Updated_Genus, Gsp, Ocean, Sed_exposure_days, Sed_days, Sed_origin, Sed_grain_class, Binary_sed_burial) %>% 
...
PR_ratio <- photosynthesisDS %>% 
  select(Ref, Ref_name, Control_code, Sed_level_mg_cm2_d, Binary_reduced_PR_ratio, Updated_Genus, Gsp, Ocean, Sed_exposure_days, Sed_days, Sed_origin, Sed_grain_class, Binary_sed_burial) %>% 
  filter(Binary_sed_burial=="0") %>% #remove all studies that buried corals instead of giving them a specific dosage (in mg/cm2/day)
...
#Photosynthetic efficiency, plotted by exposure concentration (x) and exposure duration (y)
photo_effSS <- photoSS %>% 
  select(Ref, Ref_name, Control_code, Sed_level_mg_L_num, Binary_reduced_FvFm, Updated_Genus, Gsp, Ocean, Sed_exposure_days) %>% 
...
#P/R ratio, plotted by exposure concentration (x) and exposure duration (y)
PR_ratioSS <- photoSS %>% 
  select(Ref, Ref_name, Control_code, Sed_level_mg_L_num, Binary_reduced_PR_ratio, Updated_Genus, Gsp, Ocean, Sed_exposure_days) %>% 
...
**Legend**: Reduced photosynthetic efficiency due to sedimentation? Empty circles are no (0), and filled squares are yes (1). Colors are for different genera.
  
**Deposited sediment**: All species, n=324 datapoints, n=8 studies, n=12 genera
```{r, echo=FALSE, warning=FALSE}
photo_eff_plot2 <- ggplot(data=photo_eff) +
    ...
    theme_bw()
photo_eff_plotly <- ggplotly(photo_eff_plot2, tooltip="text")
photo_eff_plotly #plot where I can hover over points and get species, but legend cut-off
```
  
**Suspended sediment**: All species, n=238 datapoints, n=5 studies, n=6 genera
```{r, echo=FALSE, warning=FALSE}
photo_effSS_plot2 <- ggplot(data=photo_effSS) +
    geom_point(mapping = aes(
      x = Sed_level_mg_L_num,
      y = Sed_exposure_days,
      shape = factor(Binary_reduced_FvFm),
    ...
    scale_shape_manual(breaks = c("0","1"), values = c(21,15)) +
    theme_bw()
photo_effSS_plotly <- ggplotly(photo_effSS_plot2, tooltip="text")
photo_effSS_plotly #plot where I can hover over points and get species, but legend cut-off
```
  
  
##### P/R ratio
**Legend**: Reduced P/R ratio due sedimentation? Empty circles are no (0), and filled squares are yes (1). Colors are for different genera.
  
**Deposited sediment**: All species, n=46 datapoints, n=4 studies, n=14 genera
```{r, echo=FALSE, warning=FALSE}
PR_ratio_plot2 <- ggplot(data=PR_ratio) +
    geom_point(mapping = aes(
      x = Sed_level_mg_cm2_d, 
      y = Sed_days,
      shape = factor(Binary_reduced_PR_ratio),
    ...
    labs(x = "Sedimentation rate (mg/cm2/d)", 
         y = "Exposure duration (days)",
         shape = "Reduced P/R ratio due\nto sedimentation?",
         color = "Genus") +
    scale_x_continuous(limits = c(0,65)) +
    scale_y_continuous(limits = c(0,30)) +
    scale_shape_manual(breaks = c("0","1"), values = c(21,15)) +
    theme_bw()
PR_ratio_plotly <- ggplotly(PR_ratio_plot2, tooltip="text")
#PR_ratio_plotly #plot where I can hover over points and get species, but legend cut-off
#hmmmm, an error: Error in gsub("\n", br(), a, fixed = TRUE) : input string 1 is invalid in this locale
```
**NOTE**: Having trouble getting this figure to work. Needs attention. Error: "Error in gsub("\n", br(), a, fixed = TRUE) : input string 1 is invalid in this locale"    
  
**Suspended sediment**: All species, n=49 datapoints, n=3 studies, n=4 genera
```{r, echo=FALSE, warning=FALSE}
PR_ratioSS_plot2 <- ggplot(data=PR_ratioSS) +
    geom_point(mapping = aes(
      x = Sed_level_mg_L_num, 
      y = Sed_exposure_days,
      shape = factor(Binary_reduced_PR_ratio),
    ...
    scale_shape_manual(breaks = c("0","1"), values = c(21,15)) +
    theme_bw()
PR_ratioSS_plotly <- ggplotly(PR_ratioSS_plot2, tooltip="text")
PR_ratioSS_plotly #plot where I can hover over points and get species, but legend cut-off
```
  
  
### *Montipora* spp. only for Deposited Sediment
*Montipora* is the genus with the most data and appears to be among the most sensitive, so let's explore it individually for photosynthetic efficiency:

```{r, echo=FALSE, warning=FALSE}
eff_montipora <- photo_eff %>% 
  filter(Updated_Genus=="Montipora")
#eff_montipora %>% tally() #n=208 datapoints, tally() returns same result as count(eff_montipora)
#eff_montipora %>% count(Ref_name, sort=T) #n=4 studies, count(Ref_name) = group_by(Ref_name) %>% tally()
#eff_montipora %>% count(Gsp, sort=T) #n=6 species, same as nlevels(as.factor(eff_montipora$Gsp))
  
eff_mont_plot <- ggplot(data=eff_montipora) +
  geom_point(mapping = aes(
      x = Sed_level_mg_cm2_d, 
      y = Sed_days,
      shape = factor(Binary_reduced_FvFm),
  ...
  labs(x = "Sedimentation rate (mg/cm2/d)", 
       ...
       shape = "Reduced photosynthetic\nefficiency due\nto sedimentation?") +
  scale_shape_manual(breaks = c("0","1"), values = c(21,15)) +
  scale_y_log10(limits = c(0.1,20), breaks=c(0.1,1,10), 
                  label=c("0.1","1","10")) +
  theme_bw()


#PLOTS:
eff_mont_plot + labs(title="Photosynthetic efficiency, Montipora spp. only", 
                     subtitle = "n=208 datapoints, n=4 studies, n=6 spp.")
```

##### Photosynthetic efficiency, Montipora spp. only, n=208 datapoints, n=4 studies, n=6 spp.
**Legend**: Reduced photosynthetic efficiency due to sedimentation? Empty circles are no (0), and filled squares are yes (1). Colors are for different genera.
```{r, echo=FALSE, warning=FALSE}
#PLOTS:
eff_mont_plotly <- ggplotly(eff_mont_plot, tooltip= c("x", "y", "text"))
eff_mont_plotly
```
...
adverse_photo_eff <- photo_eff %>% 
  ...
  arrange(Sed_level_mg_cm2_d) #minimum Sedimentation level is 25

#LOAEL
summarize(adverse_photo_eff, LOAEL=min(Sed_level_mg_cm2_d)) #LOAEL is 25 and comes from Flores et al. (2012).
loaelDS1 <- adverse_photo_eff %>% summarize(min(Sed_level_mg_cm2_d))
...
non_adverse_photo_eff <- photo_eff %>%
  ...
  filter(Sed_level_mg_cm2_d<=25) #less than or equal to the LOAEL, which = 25

#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_photo_eff, NOAEL=max(Sed_level_mg_cm2_d)) #NOAEL is 25

#PLOT
photosynthesisDS %>% 
  select(Ocean, Region, Genus, species, Binary_sed_burial, Sed_level_mg_cm2_d, Binary_reduced_FvFm) %>% 
  filter(Binary_sed_burial=="0") %>% #remove all studies that buried corals instead of giving them a specific dosage (in mg/cm2/day)
  ggplot() +
    geom_point(mapping = aes(
      x = Sed_level_mg_cm2_d, 
...
#LOAEL
summarize(adverse_photo_eff2, LOAEL=min(Sed_days)) #LOAEL is 0.5 days = 12 h
...
#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_photo_eff2, NOAEL=max(Sed_days)) #NOAEL is 0.5 days
...
#LOAEL
summarize(adverse_PR_ratio, LOAEL=min(Sed_level_mg_cm2_d)) #LOAEL is 26.4 and comes from Hodgson (1989b) Chapter IV - field component.
loaelDS2 <- adverse_PR_ratio %>% summarize(min(Sed_level_mg_cm2_d))
...
non_adverse_PR_ratio <- PR_ratio %>%
  ...
  arrange(desc(Sed_level_mg_cm2_d)) %>%
  filter(Sed_level_mg_cm2_d<=26.4) #less than or equal to the LOAEL, which = 26.4

#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_PR_ratio, NOAEL=max(Sed_level_mg_cm2_d)) #NOAEL is 26.4

#PLOT
photosynthesisDS %>% 
  select(Ocean, Region, Genus, species, Binary_sed_burial, Sed_level_mg_cm2_d, Binary_reduced_PR_ratio) %>% 
  filter(Binary_sed_burial=="0") %>% #remove all studies that buried corals instead of giving them a specific dosage (in mg/cm2/day)
  ggplot() +
    geom_point(mapping = aes(
      x = Sed_level_mg_cm2_d, 
...
#LOAEL
summarize(adverse_PR_ratio2, LOAEL=min(Sed_days)) #LOAEL is 2 days

#Setting up for NOAEL by filtering for non-adverse effects (binary=0), and then by any sediment level less than or equal to the LOAEL
non_adverse_PR_ratio2 <- PR_ratio %>%
  filter(Binary_reduced_PR_ratio=="0") %>%
  filter(Sed_days<=2) #less than or equal to the LOAEL, which = 2

#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_PR_ratio2, NOAEL=max(Sed_days)) #NOAEL is 2 days
...
adverse_photoSS <- photo_effSS %>% 
  filter(Binary_reduced_FvFm=="1") %>%
  arrange(Sed_level_mg_L_num)

#LOAEL
summarize(adverse_photoSS, LOAEL=min(Sed_level_mg_L_num)) #LOAEL is 35.8 and is from Browne et al. (2015) with Merulina ampliata for 56 days
loaelSS1 <- adverse_photoSS %>% summarize(min(Sed_level_mg_L_num))
loaelSS1 <- as.numeric(loaelSS1)

#Setting up for NOAEL by filtering for non-adverse effects (binary=0), and then by any sediment level less than or equal to the LOAEL
non_adverse_photoSS <- photo_effSS %>%
  filter(Binary_reduced_FvFm=="0") %>%
  arrange(desc(Sed_level_mg_L_num)) %>%
  filter(Sed_level_mg_L_num<=35.8) #less than or equal to the LOAEL, which = 35.8

#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_photoSS, NOAEL=max(Sed_level_mg_L_num)) #NOAEL is 35.8

#PLOT
photo_effSS %>% 
  select(Ocean, Gsp, Sed_level_mg_L_num, Binary_reduced_FvFm) %>% 
  ggplot() +
    geom_point(mapping = aes(
      x = Sed_level_mg_L_num, 
      y = Binary_reduced_FvFm)) + 
...
#LOAEL
summarize(adverse_pr_ratioSS, LOAEL=min(Sed_level_mg_L_num)) #LOAEL is 35.8 and is from Browne et al. (2015) with Merulina ampliata for 56 days
loaelSS2 <- adverse_pr_ratioSS %>% summarize(min(Sed_level_mg_L_num))
...
#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_pr_ratioSS, NOAEL=max(Sed_level_mg_L_num)) #NOAEL is 35.8

#PLOT
PR_ratioSS %>% 
  select(Ocean, Gsp, Sed_level_mg_L_num, Binary_reduced_PR_ratio) %>% 
  ggplot() +
    geom_point(mapping = aes(
      x = Sed_level_mg_L_num, 
      y = Binary_reduced_PR_ratio)) + 
...
modDS1 <- glm(Binary_reduced_FvFm ~ Sed_level_mg_cm2_d, 
            family = binomial(link = "logit"), data = photo_effDS2)
modDS2 <- glm(Binary_reduced_FvFm ~ Sed_level_mg_cm2_d + Sed_days, 
            family = binomial(link = "logit"), data = photo_effDS2)
modDS3 <- glm(Binary_reduced_FvFm ~ Sed_level_mg_cm2_d*Sed_days, 
            family = binomial(link = "logit"), data = photo_effDS2)

#models with random effect for coral species or study
modDS4 <- glmer(Binary_reduced_FvFm ~ Sed_level_mg_cm2_d + (1 | Gsp), 
              family = binomial, data = photo_effDS2)
modDS5 <- glmer(Binary_reduced_FvFm ~ Sed_level_mg_cm2_d + Sed_days + (1 | Gsp), 
              family = binomial, data = photo_effDS2)
modDS6 <- glmer(Binary_reduced_FvFm ~ Sed_level_mg_cm2_d*Sed_days + (1 | Gsp), 
              family = binomial, data = photo_effDS2)
modDS7 <- glmer(Binary_reduced_FvFm ~ Sed_level_mg_cm2_d + (1 | Ref), 
              family = binomial, data = photo_effDS2)
modDS8 <- glmer(Binary_reduced_FvFm ~ Sed_level_mg_cm2_d + Sed_days + (1 | Ref), 
              family = binomial, data = photo_effDS2)
modDS9 <- glmer(Binary_reduced_FvFm ~ Sed_level_mg_cm2_d*Sed_days + (1 | Ref), 
              family = binomial, data = photo_effDS2)
modDS10 <- glmer(Binary_reduced_FvFm ~ Sed_level_mg_cm2_d + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = photo_effDS2)
...
modDS7_noSed <- glmer(Binary_reduced_FvFm ~ 1 + (1 | Ref), 
                    family = binomial, data = photo_effDS2)
#Explained_var_Ref <- (1-VarCorr(modDS7)$Ref[1]/VarCorr(modDS7_noSed)$Ref[1]) #not returning a proportion of variance explained, so not sure how to interpret.
...
for (i in 1:length(random.intercepts)) {
  if (i == 1) curve(exp(Ref.intercepts[i] + fixef(modDS7)["Sed_level_mg_cm2_d"]*x)/(1 + exp(Ref.intercepts[i] + fixef(modDS7)["Sed_level_mg_cm2_d"]*x)), 
                    from = min(photo_effDS2$Sed_level_mg_cm2_d), to = max(photo_effDS2$Sed_level_mg_cm2_d), 
                    ylim = c(0, 1), 
                    ylab = "Probability of adverse effect", 
                    xlab = "Exposure concentration, mg/cm2/d")
  if (i > 1) curve(exp(Ref.intercepts[i] + fixef(modDS7)["Sed_level_mg_cm2_d"]*x)/(1 + exp(Ref.intercepts[i] + fixef(modDS7)["Sed_level_mg_cm2_d"]*x)), add = T) 
}
...
photo_effDS_plot <- ggplot(data = photo_effDS3) +
    ...
    scale_x_log10(limits=c(0.01,1000),breaks=c(0.01,0.1,1,10,100,1000),
                  label=c("0.01","0.1","1","10","100","1000")) +
    geom_line(aes(x = Sed_level_mg_cm2_d, y = pred_modDS10), inherit.aes=FALSE) +
    theme_bw()
...
```{r}
jvalues <- with(photo_effDS2, seq(from = min(Sed_level_mg_cm2_d), to = max(Sed_level_mg_cm2_d), length.out = 100))

# calculate predicted probabilities and store in a list
pp <- lapply(jvalues, function(j) {
    photo_effDS2$Sed_level_mg_cm2_d <- j
...
#Models with fixed effect(s) only
modDSb1 <- glm(Binary_reduced_PR_ratio ~ Sed_level_mg_cm2_d, 
            family = binomial(link = "logit"), data = PR_ratioDS2)
modDSb2 <- glm(Binary_reduced_PR_ratio ~ Sed_level_mg_cm2_d + Sed_days, 
            family = binomial(link = "logit"), data = PR_ratioDS2)
modDSb3 <- glm(Binary_reduced_PR_ratio ~ Sed_level_mg_cm2_d*Sed_days, 
            family = binomial(link = "logit"), data = PR_ratioDS2)

#models with random effect for coral species or study
modDSb4 <- glmer(Binary_reduced_PR_ratio ~ Sed_level_mg_cm2_d + (1 | Gsp), 
              family = binomial, data = PR_ratioDS2)
modDSb5 <- glmer(Binary_reduced_PR_ratio ~ Sed_level_mg_cm2_d + Sed_days + (1 | Gsp), 
              family = binomial, data = PR_ratioDS2)
modDSb6 <- glmer(Binary_reduced_PR_ratio ~ Sed_level_mg_cm2_d*Sed_days + (1 | Gsp), 
              family = binomial, data = PR_ratioDS2)
modDSb7 <- glmer(Binary_reduced_PR_ratio ~ Sed_level_mg_cm2_d + (1 | Ref), 
              family = binomial, data = PR_ratioDS2)
modDSb8 <- glmer(Binary_reduced_PR_ratio ~ Sed_level_mg_cm2_d + Sed_days + (1 | Ref), 
              family = binomial, data = PR_ratioDS2)
modDSb9 <- glmer(Binary_reduced_PR_ratio ~ Sed_level_mg_cm2_d*Sed_days + (1 | Ref), 
              family = binomial, data = PR_ratioDS2)
modDSb10 <- glmer(Binary_reduced_PR_ratio ~ Sed_level_mg_cm2_d + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = PR_ratioDS2)
...
modDSb8_noSed <- glmer(Binary_reduced_PR_ratio ~ 1 + (1 | Ref), 
                    family = binomial, data = PR_ratioDS2)
#Explained_var_Ref <- (1-VarCorr(modDSb8)$Ref[1]/VarCorr(modDSb8_noSed)$Ref[1]) #not returning a proportion of variance explained, so not sure how to interpret.
random.intercepts <- ranef(modDSb8)$Ref[["(Intercept)"]]
Ref.intercepts <- fixef(modDSb8)[1] + random.intercepts 
#plot with probability of binary response on y-axis, each line representing a study:
for (i in 1:length(random.intercepts)) {
  if (i == 1) curve(exp(Ref.intercepts[i] + fixef(modDSb8)["Sed_level_mg_cm2_d"]*x)/(1 + exp(Ref.intercepts[i] + fixef(modDSb8)["Sed_level_mg_cm2_d"]*x)), 
                    from = min(PR_ratioDS2$Sed_level_mg_cm2_d), to = max(PR_ratioDS2$Sed_level_mg_cm2_d), 
                    ylim = c(0, 1), 
                    ylab = "Probability of adverse effect", 
                    xlab = "Exposure concentration, mg/cm2/d")
  if (i > 1) curve(exp(Ref.intercepts[i] + fixef(modDSb8)["Sed_level_mg_cm2_d"]*x)/(1 + exp(Ref.intercepts[i] + fixef(modDSb8)["Sed_level_mg_cm2_d"]*x)), add = T) 
}
...
PR_ratioDS_plot <- ggplot(data = PR_ratioDS3) +
    ...
    labs(x = expression("Sediment exposure concentration (mg/cm"^"2"*"/day)"), 
         ...
         color = "Study") +
    scale_x_continuous(limits=c(0,200)) +
    geom_line(aes(x = Sed_level_mg_cm2_d, y = pred_modDSb8), inherit.aes=FALSE) +
    theme_bw()
...
```{r}
jvalues <- with(PR_ratioDS2, seq(from = min(Sed_level_mg_cm2_d), to = max(Sed_level_mg_cm2_d), length.out = 100))

# calculate predicted probabilities and store in a list
pp <- lapply(jvalues, function(j) {
    PR_ratioDS2$Sed_level_mg_cm2_d <- j
...
```{r}
jvalues <- with(PR_ratioDS2, seq(from = min(Sed_days), to = max(Sed_days), length.out = 100))

# calculate predicted probabilities and store in a list
pp <- lapply(jvalues, function(j) {
    PR_ratioDS2$Sed_days <- j
    predict(modDSb8, newdata = PR_ratioDS2, type = "response")
})
...
# add in Sed_days values and convert to data frame
plotdat <- as.data.frame(cbind(plotdat, jvalues))
...
photo_effSS2 <- photo_effSS %>% 
  filter(Control_code=="0") %>% 
  mutate(log10_conc=log10(Sed_level_mg_L_num)) %>% 
  mutate(log10_dur=log10(Sed_exposure_days))
photo_effSS2 %>% tally()

#Models with fixed effect(s) only
modSS1 <- glm(Binary_reduced_FvFm ~ Sed_level_mg_L_num, 
            family = binomial(link = "logit"), data = photo_effSS2)
modSS2 <- glm(Binary_reduced_FvFm ~ Sed_level_mg_L_num + Sed_exposure_days, 
            family = binomial(link = "logit"), data = photo_effSS2)
modSS3 <- glm(Binary_reduced_FvFm ~ Sed_level_mg_L_num*Sed_exposure_days, 
            family = binomial(link = "logit"), data = photo_effSS2)

#models with random effect for coral species or study
modSS4 <- glmer(Binary_reduced_FvFm ~ Sed_level_mg_L_num + (1 | Gsp), 
              family = binomial, data = photo_effSS2)
modSS5 <- glmer(Binary_reduced_FvFm ~ 
                  Sed_level_mg_L_num + Sed_exposure_days + (1 | Gsp), 
              family = binomial, data = photo_effSS2)
modSS6 <- glmer(Binary_reduced_FvFm ~ 
                  Sed_level_mg_L_num*Sed_exposure_days + (1 | Gsp),
                family = binomial, data = photo_effSS2)
modSS7 <- glmer(Binary_reduced_FvFm ~ Sed_level_mg_L_num + (1 | Ref), 
              family = binomial, data = photo_effSS2)
modSS8 <- glmer(Binary_reduced_FvFm ~ 
                  Sed_level_mg_L_num + Sed_exposure_days + (1 | Ref), 
                family = binomial, data = photo_effSS2)
modSS9 <- glmer(Binary_reduced_FvFm ~ 
                  Sed_level_mg_L_num*Sed_exposure_days + (1 | Ref),
                family = binomial, data = photo_effSS2)
modSS10 <- glmer(Binary_reduced_FvFm ~ Sed_level_mg_L_num + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = photo_effSS2)
modSS11 <- glmer(Binary_reduced_FvFm ~ 
                Sed_level_mg_L_num + Sed_exposure_days + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = photo_effSS2)
modSS12 <- glmer(Binary_reduced_FvFm ~ 
                Sed_level_mg_L_num*Sed_exposure_days + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = photo_effSS2)
modSS13 <- glmer(Binary_reduced_FvFm ~ Sed_level_mg_L_num + (1 | Ref_name/Ref), 
              family = binomial, data = photo_effSS2)
modSS14 <- glmer(Binary_reduced_FvFm ~ 
                 Sed_level_mg_L_num + Sed_exposure_days + (1 | Ref_name/Ref), 
              family = binomial, data = photo_effSS2)
modSS15 <- glmer(Binary_reduced_FvFm ~ 
                 Sed_level_mg_L_num*Sed_exposure_days + (1 | Ref_name/Ref), 
              family = binomial, data = photo_effSS2)
modSS16 <- glmer(Binary_reduced_FvFm ~ 
                 Sed_level_mg_L_num + (1 | Gsp) + (1 | Ref_name), 
              family = binomial, data = photo_effSS2)
modSS17 <- glmer(Binary_reduced_FvFm ~ 
                 Sed_level_mg_L_num + Sed_exposure_days + (1 | Gsp) + (1 | Ref_name), 
              family = binomial, data = photo_effSS2)
modSS18 <- glmer(Binary_reduced_FvFm ~ 
                 Sed_level_mg_L_num*Sed_exposure_days + (1 | Gsp) + (1 | Ref_name), 
              family = binomial, data = photo_effSS2)
modSS19 <- glmer(Binary_reduced_FvFm ~ 
                 Sed_level_mg_L_num + (1 | Gsp) + (1 | Ref_name/Ref), 
              family = binomial, data = photo_effSS2)
modSS20 <- glmer(Binary_reduced_FvFm ~ 
                 Sed_level_mg_L_num + Sed_exposure_days + (1 | Gsp) + (1 | Ref_name/Ref), 
              family = binomial, data = photo_effSS2)
...
modSS5_noSed <- glmer(Binary_reduced_FvFm ~ 1 + (1 | Gsp), 
                    family = binomial, data = photo_effSS2)
#Explained_var_Gsp <- (1-VarCorr(modSS5)$Gsp[1]/VarCorr(modSS5_noSed)$Gsp[1]) #not returning a proportion of variance explained, so not sure how to interpret.
random.intercepts <- ranef(modSS5)$Gsp[["(Intercept)"]]
Gsp.intercepts <- fixef(modSS5)[1] + random.intercepts 
#plot with probability of binary response on y-axis, each line representing a study:
for (i in 1:length(random.intercepts)) {
  if (i == 1) curve(exp(Gsp.intercepts[i] + fixef(modSS5)["Sed_level_mg_L_num"]*x)/(1 + exp(Gsp.intercepts[i] + fixef(modSS5)["Sed_level_mg_L_num"]*x)), 
                    from = min(photo_effSS2$Sed_level_mg_L_num), to = max(photo_effSS2$Sed_level_mg_L_num), 
                    ylim = c(0, 1), 
                    ylab = "Probability of adverse effect", 
                    xlab = "Exposure concentration, mg/cm2/d")
  if (i > 1) curve(exp(Gsp.intercepts[i] + fixef(modSS5)["Sed_level_mg_L_num"]*x)/(1 + exp(Gsp.intercepts[i] + fixef(modSS5)["Sed_level_mg_L_num"]*x)), add = T) 
}
...
photo_effSS_plot <- ggplot(data = photo_effSS3) +
    ...
    scale_x_log10(limits=c(1,250),breaks=c(0.01,0.1,1,10,100,1000),
                  label=c("0.01","0.1","1","10","100","1000")) +
    geom_line(aes(x = Sed_level_mg_L_num, y = pred_modSS5), inherit.aes=FALSE) +
    theme_bw()
...
```{r}
jvalues <- with(photo_effSS2, seq(from = min(Sed_level_mg_L_num), to = max(Sed_level_mg_L_num), length.out = 100))

# calculate predicted probabilities and store in a list
pp <- lapply(jvalues, function(j) {
    photo_effSS2$Sed_level_mg_L_num <- j
...
# add in Sed_level_mg_L_num values and convert to data frame
plotdat <- as.data.frame(cbind(plotdat, jvalues))
...
photo_effSS_plot2 <- ggplot() +
    ...
    labs(x = "Sediment exposure concentration (mg/L)", 
         ...
         color = "Binary Data\nby Study",
         linetype = "Predicted\nProbability") +
    scale_x_log10(limits=c(1,max(photo_effSS2$Sed_level_mg_L_num)), 
...
#Models with fixed effect(s) only
modSSb1 <- glm(Binary_reduced_PR_ratio ~ Sed_level_mg_L_num, 
            family = binomial(link = "logit"), data = PR_ratioSS2)
modSSb2 <- glm(Binary_reduced_PR_ratio ~ Sed_level_mg_L_num + Sed_exposure_days, 
            family = binomial(link = "logit"), data = PR_ratioSS2)
modSSb3 <- glm(Binary_reduced_PR_ratio ~ Sed_level_mg_L_num*Sed_exposure_days, 
            family = binomial(link = "logit"), data = PR_ratioSS2)

#models with random effect for coral species or study
modSSb4 <- glmer(Binary_reduced_PR_ratio ~ Sed_level_mg_L_num + (1 | Gsp), 
...
modSSb6 <- glmer(Binary_reduced_PR_ratio ~ 
                  Sed_level_mg_L_num*Sed_exposure_days + (1 | Gsp),
                family = binomial, data = PR_ratioSS2)
modSSb7 <- glmer(Binary_reduced_PR_ratio ~ Sed_level_mg_L_num + (1 | Ref), 
              family = binomial, data = PR_ratioSS2)
...
modSSb9 <- glmer(Binary_reduced_PR_ratio ~ 
                  Sed_level_mg_L_num*Sed_exposure_days + (1 | Ref),
                family = binomial, data = PR_ratioSS2)
modSSb10 <- glmer(Binary_reduced_PR_ratio ~ Sed_level_mg_L_num + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = PR_ratioSS2)
...
modSSb4_noSed <- glmer(Binary_reduced_PR_ratio ~ 1 + (1 | Gsp), 
                    family = binomial, data = PR_ratioSS2)
#Explained_var_Gsp <- (1-VarCorr(modSSb4)$Gsp[1]/VarCorr(modSSb4_noSed)$Gsp[1]) #not returning a proportion of variance explained, so not sure how to interpret.
random.intercepts <- ranef(modSSb4)$Gsp[["(Intercept)"]]
Gsp.intercepts <- fixef(modSSb4)[1] + random.intercepts 
#plot with probability of binary response on y-axis, each line representing a study:
for (i in 1:length(random.intercepts)) {
  if (i == 1) curve(exp(Gsp.intercepts[i] + fixef(modSSb4)["Sed_level_mg_L_num"]*x)/(1 + exp(Gsp.intercepts[i] + fixef(modSSb4)["Sed_level_mg_L_num"]*x)), 
                    from = min(PR_ratioSS2$Sed_level_mg_L_num), to = max(PR_ratioSS2$Sed_level_mg_L_num), 
                    ylim = c(0, 1), 
                    ylab = "Probability of adverse effect", 
                    xlab = "Exposure concentration, mg/cm2/d")
  if (i > 1) curve(exp(Gsp.intercepts[i] + fixef(modSSb4)["Sed_level_mg_L_num"]*x)/(1 + exp(Gsp.intercepts[i] + fixef(modSSb4)["Sed_level_mg_L_num"]*x)), add = T) 
}
...
PR_ratioSS_plot <- ggplot(data = PR_ratioSS3) +
    ...
    scale_x_log10(limits=c(10,1000),breaks=c(0.01,0.1,1,10,100,1000),
                  label=c("0.01","0.1","1","10","100","1000")) +
    geom_line(aes(x = Sed_level_mg_L_num, y = pred_modSSb4), inherit.aes=FALSE) +
    theme_bw()
...
##### By sediment exposure concentration
```{r}
jvalues <- with(PR_ratioSS2, seq(from = min(Sed_level_mg_L_num), to = max(Sed_level_mg_L_num), length.out = 100))

# calculate predicted probabilities and store in a list
pp <- lapply(jvalues, function(j) {
    PR_ratioSS2$Sed_level_mg_L_num <- j
...
PR_ratioSS_plot2 <- ggplot() +
    ...
    labs(x = "Sediment exposure concentration (mg/L)", 
         ...
         color = "Binary Data\nby Study",
         linetype = "Predicted\nProbability") +
    scale_x_log10(limits=c(10,max(PR_ratioSS2$Sed_level_mg_L_num)), 
