---
title: "Effects of sediment on bleaching of adult corals"
...
#head(bleach)
bleachingDS <- binaryDS %>%
  filter(!is.na(Binary_bleaching)) %>% 
...
bleach <- bleachingDS %>% 
  select(Ref, Ref_name, Control_code, Sed_level_mg_cm2_d, Binary_bleaching, Updated_Genus, Gsp, Ocean, Sed_exposure_days, Sed_days, Sed_origin, Sed_grain_class, Binary_sed_burial) %>% 
  filter(Binary_sed_burial=="0") %>% #remove all studies that buried corals instead of giving them a specific dosage (in mg/cm2/day)
  filter(!is.na(Binary_bleaching)) #remove NAs (studies that did not describe photosynthetic efficiency)
...
bleach_plot <- ggplot(data=bleach, mapping=aes(
      ...
      y = Sed_days,
      color = factor(Binary_bleaching),
      shape = factor(Binary_bleaching),
      position = "jitter",
      text = Gsp)) +
    geom_point(data = subset(bleach, Binary_bleaching=="0")) + 
...
bleachSS <- bleachingSS %>% 
  select(Ref, Ref_name, Control_code, Sed_level_mg_L, Binary_bleaching_or_reduced_chla, Updated_Genus, Gsp, Ocean, Sed_exposure_days)
...
#PLOTS:
bleach_plot + geom_point(data = subset(bleach, Binary_bleaching=="1")) +
...
##### Bleaching
**Legend**: Bleaching due to sedimentation? Empty circles are no (0), and filled squares are yes (1). Colors are for different genera.  

**Deposited sediment**: All species, n=385 datapoints, n=19 studies, n=32 genera
```{r, echo=FALSE, warning=FALSE}
bleach_plot2 <- ggplot(data=bleach) +
    ...
    theme_bw()
bleach_plotly <- ggplotly(bleach_plot2, tooltip="text")
bleach_plotly #plot where I can hover over points and get species, but legend cut-off
```
  
**Suspended sediment**: All species, n=92 datapoints, n=7 studies, n=6 genera
```{r, echo=FALSE, warning=FALSE}
bleachSS_plot2 <- ggplot(data=bleachSS) +
    geom_point(mapping = aes(
      x = Sed_level_mg_L, 
      y = Sed_exposure_days,
      shape = factor(Binary_bleaching_or_reduced_chla),
    ...
    scale_shape_manual(breaks = c("0","1"), values = c(21,15)) +
    theme_bw()
bleachSS_plotly <- ggplotly(bleachSS_plot2, tooltip="text")
bleachSS_plotly #plot where I can hover over points and get species, but legend cut-off
```
  
  
### *Montipora* spp. only for Deposited Sediment
*Montipora* is the genus with the most data and appears to be among the most sensitive, so let's explore it individually for bleaching:

```{r, echo=FALSE, warning=FALSE}
bleach_montipora <- bleach %>% 
  filter(Updated_Genus=="Montipora")
#bleach_montipora %>% tally() #n=65 datapoints, tally() returns same result as count(bleach_montipora)
#bleach_montipora %>% count(Ref_name, sort=T) #n=7 studies, count(Ref_name) = group_by(Ref_name) %>% tally()
#bleach_montipora %>% count(Gsp, sort=T) #n=7 species, same as nlevels(as.factor(bleach_montipora$Gsp))
  
bleach_mont_plot <- ggplot(data=bleach_montipora) +
  geom_point(mapping = aes(
      x = Sed_level_mg_cm2_d, 
      y = Sed_days,
      shape = factor(Binary_bleaching),
...
#PLOTS:
#bleach_mont_plot + labs(title = "Bleaching, Montipora spp. only", 
#                        subtitle = "n=65 datapoints, n=7 studies, n=7 spp.")
```  
  
  
##### Bleaching, Montipora spp. only, n=65 datapoints, n=7 studies, n=7 spp.
**Legend**: Bleaching due to sedimentation? Empty circles are no (0), and filled squares are yes (1). Colors are for different genera.
```{r, echo=FALSE, warning=FALSE}
#PLOTS:
bleach_mont_plotly <- ggplotly(bleach_mont_plot, tooltip= c("x", "y", "text"))
bleach_mont_plotly
```
  
  
### *Acropora* spp. only for Suspended Sediment 
*Acropora* is the genus with the most data and appears to be among the most sensitive, so let's explore it individually for bleaching:

```{r, echo=FALSE, warning=FALSE}
bleach_Acropora <- bleachSS %>% 
  filter(Updated_Genus=="Acropora")
#bleach_Acropora %>% tally() #n=49 datapoints, tally() returns same result as count(bleach_Acropora)
#bleach_Acropora %>% count(Ref_name, sort=T) #n=4 studies, count(Ref_name) = group_by(Ref_name) %>% tally()
#bleach_Acropora %>% count(Gsp, sort=T) #n=3 species, same as nlevels(as.factor(bleach_Acropora$Gsp))
  
bleach_acro_plot <- ggplot(data=bleach_Acropora) +
  geom_point(mapping = aes(
      x = Sed_level_mg_L, 
      y = Sed_exposure_days,
      shape = factor(Binary_bleaching_or_reduced_chla),
...
#PLOTS:
#bleach_acro_plot + labs(title = "Bleaching, Acropora spp. only", 
#                        subtitle ="n=49 datapoints, n=4 studies, n=3 spp.")
```  
  
  
##### Bleaching, Acropora spp. only, n=49 datapoints, n=4 studies, n=3 spp.
**Legend**: Bleaching due to sedimentation? Empty circles are no (0), and filled squares are yes (1). Colors are for different genera.
```{r, echo=FALSE, warning=FALSE}
#PLOTS:
bleach_acro_plotly <- ggplotly(bleach_acro_plot, tooltip= c("x", "y", "text"))
bleach_acro_plotly
```
...
#LOAEL
summarize(adverse_bleach, LOAEL=min(Sed_level_mg_cm2_d)) #LOAEL is 4.9 and comes from Sofonia (2006) PhD Chapter 4
loael <- adverse_bleach %>% summarize(min(Sed_level_mg_cm2_d))
...
summarize(non_adverse_bleach, NOAEL=max(Sed_level_mg_cm2_d)) #NOAEL is 4.9

#PLOT
#bleaching %>% 
#  select(Ocean, Region, Genus, species, Binary_sed_burial, Sed_level_mg_cm2_d, #Binary_bleaching) %>% 
#  filter(Binary_sed_burial=="0") %>% #remove all studies that buried corals instead of #giving them a specific dosage (in mg/cm2/day)
#  ggplot() +
#    geom_point(mapping = aes(
#      x = Sed_level_mg_cm2_d, 
...
#LOAEL
summarize(adverse_bleach2, LOAEL=min(Sed_days)) #LOAEL is 0.92 days = 22h

#Setting up for NOAEL by filtering for non-adverse effects (binary=0), and then by any sediment level less than or equal to the LOAEL
non_adverse_bleach2 <- bleach %>%
  filter(Binary_bleaching=="0") %>%
  filter(Sed_days<=0.92) #less than or equal to the LOAEL, which = 21

#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_bleach2, NOAEL=max(Sed_days)) #NOAEL is 0.92 days
...
#LOAEL
summarize(adverse_bleachSS, LOAEL=min(Sed_level_mg_L)) #LOAEL is 3.22 and comes from Flores et al. (2012) with Acropora millepora for 84 days exposure
loaelSS <- adverse_bleachSS %>% summarize(min(Sed_level_mg_L))
...
#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_bleachSS, NOAEL=max(Sed_level_mg_L)) #NOAEL is 3.22

#PLOT
bleachingSS %>% 
  select(Ocean, Gsp, Sed_level_mg_L, Binary_bleaching_or_reduced_chla) %>% 
  ggplot() +
    geom_point(mapping = aes(
      x = Sed_level_mg_L,
...
modDS1 <- glm(Binary_bleaching ~ Sed_level_mg_cm2_d, 
            family = binomial(link = "logit"), data = bleachDS2)
modDS2 <- glm(Binary_bleaching ~ Sed_level_mg_cm2_d + Sed_days, 
            family = binomial(link = "logit"), data = bleachDS2)
modDS3 <- glm(Binary_bleaching ~ Sed_level_mg_cm2_d*Sed_days, 
            family = binomial(link = "logit"), data = bleachDS2)

#models with random effect for coral species or study
modDS4 <- glmer(Binary_bleaching ~ Sed_level_mg_cm2_d + (1 | Gsp), 
              family = binomial, data = bleachDS2)
modDS5 <- glmer(Binary_bleaching ~ Sed_level_mg_cm2_d + Sed_days + (1 | Gsp), 
              family = binomial, data = bleachDS2)
modDS6 <- glmer(Binary_bleaching ~ Sed_level_mg_cm2_d*Sed_days + (1 | Gsp), 
              family = binomial, data = bleachDS2)
modDS7 <- glmer(Binary_bleaching ~ Sed_level_mg_cm2_d + (1 | Ref), 
              family = binomial, data = bleachDS2)
modDS8 <- glmer(Binary_bleaching ~ Sed_level_mg_cm2_d + Sed_days + (1 | Ref), 
              family = binomial, data = bleachDS2)
modDS9 <- glmer(Binary_bleaching ~ Sed_level_mg_cm2_d*Sed_days + (1 | Ref), 
              family = binomial, data = bleachDS2)
modDS10 <- glmer(Binary_bleaching ~ Sed_level_mg_cm2_d + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = bleachDS2)
modDS11 <- glmer(Binary_bleaching ~ 
                Sed_level_mg_cm2_d + Sed_days + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = bleachDS2)
modDS12 <- glmer(Binary_bleaching ~ 
                Sed_level_mg_cm2_d*Sed_days + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = bleachDS2)
...
modDS10_log <- glmer(Binary_bleaching ~ log10_conc + (1 | Gsp) + (1 | Ref), 
                     family = binomial, data = bleachDS2)
...
#proportion correct:
mean(p==bleachDS2$Binary_bleaching) #[1] 0.9488636
#predicted-vs-observed table:
table(p,bleachDS2$Binary_bleaching)
#ROC and area under the curve:
pred_modDS10_log <- predict(modDS10_log, newdata = bleachDS2, type = "response")
auc(roc(bleachDS2$Binary_bleaching, pred_modDS10_log)) #AUC: 0.9888
...
modDS10_log_noSed <- glmer(Binary_bleaching ~ 1 + (1 | Gsp) + (1 | Ref), 
                           family = binomial, data = bleachDS2)
#Explained_var_Ref <- (1-VarCorr(modDS10_log)$Ref[1]/VarCorr(modDS10_log_noSed)$Ref[1]) #not returning a proportion of variance explained, so not sure how to interpret.
...
bleachDS_plot <- ggplot(data = bleachDS3) +
    ...
    scale_x_log10(limits=c(1,1000),breaks=c(0.01,0.1,1,10,100,1000),
                  label=c("0.01","0.1","1","10","100","1000")) +
    geom_line(aes(x = Sed_level_mg_cm2_d, y = pred_modDS10_log), inherit.aes=FALSE) +
    theme_bw()
...
#Models with fixed effect(s) only
modSS1 <- glm(Binary_bleaching_or_reduced_chla ~ Sed_level_mg_L, 
            family = binomial(link = "logit"), data = bleachSS2)
modSS2 <- glm(Binary_bleaching_or_reduced_chla ~ Sed_level_mg_L + Sed_exposure_days, 
            family = binomial(link = "logit"), data = bleachSS2)
modSS3 <- glm(Binary_bleaching_or_reduced_chla ~ Sed_level_mg_L*Sed_exposure_days, 
            family = binomial(link = "logit"), data = bleachSS2)

#models with random effect for coral species or study
modSS4 <- glmer(Binary_bleaching_or_reduced_chla ~ Sed_level_mg_L + (1 | Gsp), 
...
modSS7 <- glmer(Binary_bleaching_or_reduced_chla ~ Sed_level_mg_L + (1 | Ref), 
              family = binomial, data = bleachSS2)
...
#                family = binomial, data = bleachSS2)
#Error in pwrssUpdate(pp, resp, tol = tolPwrss, GQmat = GQmat, compDev = compDev, : Downdated VtV is not positive definite
modSS10 <- glmer(Binary_bleaching_or_reduced_chla ~ Sed_level_mg_L + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = bleachSS2)
...
modSS13 <- glmer(Binary_bleaching_or_reduced_chla ~ Sed_level_mg_L + (1 | Ref_name/Ref), 
              family = binomial, data = bleachSS2)
...
#Sed_level_mg_L    1.012222478 1.004126e+00 1.02563852
...
bleachSS_plot <- ggplot(data = bleachSS3) +
    ...
    scale_x_log10(limits=c(1,1000),breaks=c(0.01,0.1,1,10,100,1000),
                  label=c("0.01","0.1","1","10","100","1000")) +
    geom_line(aes(x = Sed_level_mg_L, y = pred_modSS2), inherit.aes=FALSE) +
    theme_bw()
...
```{r}
jvalues <- with(bleachSS2, seq(from = min(Sed_level_mg_L), to = max(Sed_level_mg_L), length.out = 100))

# calculate predicted probabilities and store in a list
pp <- lapply(jvalues, function(j) {
    bleachSS2$Sed_level_mg_L <- j
...
bleachSS_plot2 <- ggplot() +
    ...
    labs(x = "Sediment exposure concentration (mg/L)", 
         ...
         color = "Binary Data\nby Study",
         linetype = "Predicted\nProbability") +
    scale_x_log10(limits=c(1,max(bleachSS2$Sed_level_mg_L)), 
