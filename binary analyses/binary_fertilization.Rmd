---
title: "Effects of sediment on coral fertilization"
...
library(lattice)
library(MuMIn)
fertilizationSS <- read.csv("/Volumes/GoogleDrive/My Drive/iScience/Code/thresholds/sedimentation/fertilization/fertilization.csv", header=T)
...
fertSS_plot1 <- ggplot(data = fertSS) +
...
fertSS_plot4 <- ggplot(data = fertSS) +
    geom_point(mapping = aes(
      x = Sed_level_mg_L, 
      y = Binary_effect_reduced_fert, size = 5)) +
    labs(x = "", y = "") +
...
#PLOTS:
fertSS_plot1 + labs(title = "Fertilization Success and Suspended Sediment", 
...
##### Fertilization, n=110 datapoints, n=6 studies, n=2 genera
**Legend**: Reduced fertilization success due to suspended sediment? Empty circles are no (0), and filled squares are yes (1). Colors are for different species.
  
```{r, echo=FALSE, warning=FALSE}
fertSS_plotly <- ggplotly(fertSS_plot2)
fertSS_plotly
```
...
adverse_fertSS <- fertSS %>% 
  filter(Binary_effect_reduced_fert=="1") %>%
  arrange(Sed_level_mg_L)

#LOAEL
summarize(adverse_fertSS, LOAEL=min(Sed_level_mg_L)) #LOAEL is 30.4 mg/L for 0.0625 days and comes from Humanes et al. 2017b: Acropora tenuis
loael <- adverse_fertSS %>% summarize(min(Sed_level_mg_L))
...
non_adverse_fertSS <- fertSS %>%
  filter(Binary_effect_reduced_fert=="0") %>%
  arrange(desc(Sed_level_mg_L)) %>%
  filter(Sed_level_mg_L<=30.4) #less than or equal to the LOAEL, which = 30.4

#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_fertSS, NOAEL=max(Sed_level_mg_L)) #NOAEL is 25
...
fertSS2 <- fertSS %>% 
  select(Ref, Ref_name, Binary_effect_reduced_fert, Control_code, Sed_level_mg_L, Sed_exposure_days, Gsp) %>% 
  mutate(log10_mg_L=log10(Sed_level_mg_L))

#Models with fixed effect(s) only
mod1 <- glm(Binary_effect_reduced_fert ~ Sed_level_mg_L, 
            family = binomial(link = "logit"), data = fertSS2)
mod2 <- glm(Binary_effect_reduced_fert ~ Sed_level_mg_L + Sed_exposure_days, 
            family = binomial(link = "logit"), data = fertSS2)
mod3 <- glm(Binary_effect_reduced_fert ~ Sed_level_mg_L*Sed_exposure_days, 
            family = binomial(link = "logit"), data = fertSS2)
...
mod4 <- glmer(Binary_effect_reduced_fert ~ Sed_level_mg_L + (1 | Gsp), 
              family = binomial, data = fertSS2)
mod5 <- glmer(Binary_effect_reduced_fert ~ Sed_level_mg_L + Sed_exposure_days + (1 | Gsp), 
              family = binomial, data = fertSS2)
mod6 <- glmer(Binary_effect_reduced_fert ~ Sed_level_mg_L*Sed_exposure_days + (1 | Gsp), 
              family = binomial, data = fertSS2)
mod7 <- glmer(Binary_effect_reduced_fert ~ Sed_level_mg_L + (1 | Ref), 
              family = binomial, data = fertSS2)
mod8 <- glmer(Binary_effect_reduced_fert ~ Sed_level_mg_L + Sed_exposure_days + (1 | Ref), 
              family = binomial, data = fertSS2)
mod9 <- glmer(Binary_effect_reduced_fert ~ Sed_level_mg_L*Sed_exposure_days + (1 | Ref), 
              family = binomial, data = fertSS2)
mod10 <- glmer(Binary_effect_reduced_fert ~ Sed_level_mg_L + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = fertSS2)
mod11 <- glmer(Binary_effect_reduced_fert ~ 
                Sed_level_mg_L + Sed_exposure_days + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = fertSS2)
mod12 <- glmer(Binary_effect_reduced_fert ~ 
                Sed_level_mg_L*Sed_exposure_days + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = fertSS2)
mod13 <- glmer(Binary_effect_reduced_fert ~ Sed_level_mg_L + (1 | Ref_name/Ref), 
              family = binomial, data = fertSS2)
mod14 <- glmer(Binary_effect_reduced_fert ~ 
                 Sed_level_mg_L + Sed_exposure_days + (1 | Ref_name/Ref), 
              family = binomial, data = fertSS2)
mod15 <- glmer(Binary_effect_reduced_fert ~ 
                 Sed_level_mg_L*Sed_exposure_days + (1 | Ref_name/Ref), 
              family = binomial, data = fertSS2)
mod16 <- glmer(Binary_effect_reduced_fert ~ 
                 Sed_level_mg_L + (1 | Gsp) + (1 | Ref_name), 
              family = binomial, data = fertSS2)
mod17 <- glmer(Binary_effect_reduced_fert ~ 
                 Sed_level_mg_L + Sed_exposure_days + (1 | Gsp) + (1 | Ref_name), 
              family = binomial, data = fertSS2)
mod18 <- glmer(Binary_effect_reduced_fert ~ 
                 Sed_level_mg_L*Sed_exposure_days + (1 | Gsp) + (1 | Ref_name), 
              family = binomial, data = fertSS2)
mod19 <- glmer(Binary_effect_reduced_fert ~ 
                 Sed_level_mg_L + (1 | Gsp) + (1 | Ref_name/Ref), 
              family = binomial, data = fertSS2)
mod20 <- glmer(Binary_effect_reduced_fert ~ 
                 Sed_level_mg_L + Sed_exposure_days + (1 | Gsp) + (1 | Ref_name/Ref), 
              family = binomial, data = fertSS2)
mod21 <- glmer(Binary_effect_reduced_fert ~ 
                 Sed_level_mg_L*Sed_exposure_days + (1 | Gsp) + (1 | Ref_name/Ref), 
              family = binomial, data = fertSS2)
...
fertSS3 %>% tally() #n=86
mod7_log <- glmer(Binary_effect_reduced_fert ~ log10_mg_L + (1 | Ref), 
                  family = binomial, data = fertSS3)
#print(summary(mod7_log), correlation=FALSE) #No errors!

#Changing to random slopes/intercept
mod7_log2 <- glmer(Binary_effect_reduced_fert ~ log10_mg_L + (log10_mg_L | Ref), 
                   family = binomial, data = fertSS3)
...
#proportion correct:
mean(p==fertSS3$Binary_effect_reduced_fert) #[1] 0.8837209
#predicted-vs-observed table:
table(p,fertSS3$Binary_effect_reduced_fert)
#ROC and area under the curve:
pred_mod7_log <- predict(mod7_log, newdata = fertSS3, type = "response")
auc(roc(fertSS3$Binary_effect_reduced_fert, pred_mod7_log)) #AUC: 0.9534
...
mod7_log_noSed <- glmer(Binary_effect_reduced_fert ~ 1 + (1 | Ref), 
                  family = binomial, data = fertSS3)
#Explained_var_Ref <- (1-VarCorr(mod7_log)$Ref[1]/VarCorr(mod7_log_noSed)$Ref[1]) #not returning a proportion of variance explained, so not sure how to interpret.
...
fertSS_plot3 <- ggplot(data = fertSS4) +
    ...
    scale_x_log10(breaks=c(0.01,0.1,1,10,100,1000),
                  label=c("0.01","0.1","1","10","100","1000")) +
    scale_y_continuous(breaks=c(0,1)) +
    geom_line(aes(x = Sed_level_mg_L, y = pred_mod7_log), inherit.aes=FALSE) +
    theme_bw() +
    scale_color_discrete(labels = c(expression(italic("Acropora digitifera")),expression(italic("Acropora millepora")),expression(italic("Acropora tenuis")),expression(italic("Pectina lactusa")))) + theme(legend.text.align = 0)
