---
title: "Effects of sediment on the survival and settlement of larval corals"
...
library(lattice)
library(MuMIn)
settlement<-read.csv("/Volumes/GoogleDrive/My Drive/iScience/Code/thresholds/sedimentation/settlement/settlement2.csv", header=T)
larvaeSS <- read.csv("/Volumes/GoogleDrive/My Drive/iScience/Code/thresholds/sedimentation/settlement/larval_survival_settleSS.csv", header=T)
...
settlement %>% tally() #n=178 datapoints, tally() returns same result as count(settlement)
settlement %>% count(Ref, sort=T) #n=6 studies
settlement %>% count(Ref_name, sort=T) #n=6 studies, count(Ref_name) = group_by(Ref_name) %>% tally()
settlement %>% count(Ocean, sort=T) #n=1 ocean (Pacific and "Indo-Pacific", which is Singapore)
settlement %>% count(Gsp, sort=T) #n=2 species
settlement %>% count(Updated_Genus, sort=T) #n=2 genera
```
...
settleSS <- larvaeSS %>%
  mutate(Gsp = paste(Updated_Genus, Updated_species, sep = "_")) %>% 
  filter(Resp1_type=="Larval settlement rate")
...
For **larval survival**, we will explore the results of 50 datapoints from 4 studies conducted in 1 ocean on 4 species within 2 genera. 
```{r, echo=FALSE, warning=FALSE}
#LARVAL SURVIVAL
survivalSS <- larvaeSS %>%
  mutate(Gsp = paste(Updated_Genus, Updated_species, sep = "_")) %>% 
  filter(Resp1_type=="Larval survival rate")
survivalSS %>% tally() #n=50 datapoints, tally() returns same result as count(survivalSS)
survivalSS %>% count(Ref, sort=T) #n=4 studies
survivalSS %>% count(Ref_name, sort=T) #n=3 studies, count(Ref_name) = group_by(Ref_name) %>% tally()
survivalSS %>% count(Ocean, sort=T) #n=1 ocean
survivalSS %>% count(Gsp, sort=T) #n=4 species
...
settle <- settlement %>% 
  select(Ref, Ref_name, Control_code, Sed_level_mg_cm2_d, Binary_effect_limited_settle, Updated_Genus, Gsp, Ocean, Sed_origin, Sed_grain_class, Resp_level, Surface_direction, Sed_application, Sed_exposure, Days_after_start) %>% 
  filter(Surface_direction=="top") #for top-facing surfaces only
#settle %>% tally() #n=132 datapoints, tally() returns same result as count(settle)
settle %>% count(Ref, sort=T) #n=6 studies,
settle %>% count(Ref_name, sort=T) #n=6 studies, count(Ref_name) = group_by(Ref_name) %>% tally()
#settle %>% count(Gsp, sort=T) #n=2 species, Pocillopora damicornis n=84, Acropora millepora n=48
#settle %>% count(Updated_Genus, sort=T) #n=2 genera
#head(settle)

settle2 <- settle %>% filter(!is.na(Binary_effect_limited_settle)) #to remove regression studies that have no binary data
#settle2 %>% tally() #n=54 datapoints
settle2 %>% count(Ref, sort=T)
settle2 %>% count(Ref_name, sort=T) #4 studies (same two species)

#response is % of coral larvae recruited (survival):
settle_plot1 <- ggplot(data = settle2) +
  geom_point(mapping = aes(
    x = Sed_level_mg_cm2_d, 
    y = Resp_level,
    color = Gsp,
    shape = Sed_application)) +
  labs(x = expression("Sediment exposure concentration (mg/cm"^"2"*"/day)"), 
       y = "Larval settlement rate (%)",
       color = "Species",
       shape = "Timing of sediment\napplication") + 
...
settle_plot2 <- ggplot(data = settle2) +
    geom_point(mapping = aes(
      x = Sed_level_mg_cm2_d, 
      y = Binary_effect_limited_settle,
      color = Gsp)) +
    labs(x = expression("Sediment exposure concentration (mg/cm"^"2"*"/day)"),  
...
settle_plot3 <- ggplot(data=settle2, mapping = aes(
      x = Sed_level_mg_cm2_d, 
      y = Days_after_start,
      color = Gsp,
      shape = factor(Binary_effect_limited_settle),
      position = "jitter")) +
    geom_point(data = subset(settle2, Binary_effect_limited_settle=="0")) + 
    labs(x = expression("Sediment exposure concentration (mg/cm"^"2"*"/day)"), 
...
settleSS_plot1 <- ggplot(data = settleSS) +
  geom_point(mapping = aes(
    x = Sed_level_mg_L, 
    y = Resp1_level,
    color = Gsp)) +
...
settleSS_plot2 <- ggplot(data = settleSS) +
    geom_point(mapping = aes(
      x = Sed_level_mg_L, 
      y = Binary_effect_limited_settlement,
      color = Gsp)) +
    labs(x = "Sediment exposure concentration (mg/L)", 
...
settleSS_plot3 <- ggplot(data=settleSS, mapping = aes(
      x = Sed_level_mg_L, 
      y = Coral_age_at_end_expt_days,
      color = Gsp,
      shape = factor(Binary_effect_limited_settlement),
      position = "jitter")) +
    geom_point(data = subset(settleSS, Binary_effect_limited_settlement=="0")) + 
    labs(x = "Sediment exposure concentration (mg/L)", 
...
survivalSS_plot2 <- ggplot(data = survivalSS) +
    geom_point(mapping = aes(
      x = Sed_level_mg_L, 
      y = Binary_effect_larval_mortality,
      color = Gsp)) +
    labs(x = "Sediment exposure concentration (mg/L)", 
    ...
    scale_y_continuous(breaks=c(0,1)) +
    theme_bw() +
    scale_color_discrete(labels = c(expression(italic("Acropora digitifera")),expression(italic("Acropora millepora")),expression(italic("Acropora tenuis")),expression(italic("Pocillopora acuta")))) + theme(legend.text.align = 0)

survivalSS_plot3 <- ggplot(data=survivalSS, mapping = aes(
      x = Sed_level_mg_L, 
      y = Coral_age_at_start_expt_days,
      color = Gsp,
      shape = factor(Binary_effect_larval_mortality),
      position = "jitter")) +
    geom_point(data = subset(survivalSS, Binary_effect_larval_mortality=="0")) + 
    labs(x = "Sediment exposure concentration (mg/L)", 
...
#PLOTS:
settle_plot1 + labs(title = "Larval Settlement Rate and Deposited Sediment", 
        subtitle = "n = 54 datapoints, n = 6 studies from 6 articles, n = 2 spp. from 2 genera")
settleSS_plot1 + labs(title = "Larval Settlement Rate and Suspended Sediment", 
...
settle_plot3 + geom_point(data = subset(settle2, Binary_effect_limited_settle=="1")) +
  labs(title = "Reduced Larval Settlement from Deposited Sediment", 
       subtitle = "n = 54 datapoints, n = 6 studies from 6 articles, n = 2 spp. from 2 genera")
settleSS_plot3 + geom_point(data = subset(settleSS, Binary_effect_limited_settlement=="1")) +
  labs(title = "Reduced Larval Settlement from Suspended Sediment", 
       subtitle = "n = 30 datapoints, n = 7 studies from 4 articles, n = 4 spp. from 3 genera")

survivalSS_plot1 + labs(title = "Larval Survival Rate and Suspended Sediment", 
        subtitle = "n = 50 datapoints, n = 4 studies from 3 articles, n = 4 spp. from 2 genera")
survivalSS_plot2 + labs(title = "Larval Mortality from Suspended Sediment", 
        subtitle = "n = 50 datapoints, n = 4 studies from 3 articles, n = 4 spp. from 2 genera")
survivalSS_plot3 + geom_point(data = subset(survivalSS, Binary_effect_larval_mortality=="1")) +
  labs(title = "Larval Mortality from Suspended Sediment", 
       subtitle = "n = 50 datapoints, n = 4 studies from 3 articles, n = 4 spp. from 2 genera")


settleDS_plot4 <- ggplot(data = settle2) +
    geom_point(mapping = aes(
      x = Sed_level_mg_cm2_d, 
      y = Binary_effect_limited_settle,
      color = Gsp, size = 3)) +
...
##### Limited settlement
**Legend**: Limited due to sedimentation? Empty circles are no (0), and filled squares are yes (1). Colors are for different species.
  
**Deposited sediment**: All species, n=54 datapoints, n=4 studies, n=2 genera
```{r, echo=FALSE, warning=FALSE}
#settle_plotly <- ggplotly(settle_plot3)
#settle_plotly #plot where I can hover over points and get species, but legend cut-off
```
  
**Suspended sediment**:   
```{r, echo=FALSE, warning=FALSE}
settleSS_plotly <- ggplotly(settleSS_plot3)
settleSS_plotly #plot where I can hover over points and get species, but legend cut-off
```
  
##### Larval mortality
**Legend**: Larval mortality due to sedimentation? Empty circles are no (0), and filled squares are yes (1). Colors are for different species.
  
**Suspended sediment**: 
```{r, echo=FALSE, warning=FALSE}
survival_plotly <- ggplotly(survivalSS_plot3)
survival_plotly #plot where I can hover over points and get species, but legend cut-off
```  
...
adverse_settle <- settle %>% 
  filter(Binary_effect_limited_settle=="1") %>%
  arrange(Sed_level_mg_cm2_d)

#LOAEL
summarize(adverse_settle, LOAEL=min(Sed_level_mg_cm2_d)) #LOAEL is 1 and comes from Ricardo et al. (2017): Acropora millepora, measured 1 day after start of experiment (sediment applied to surface before larvae added)
loaelDS <- adverse_settle %>% summarize(min(Sed_level_mg_cm2_d))
...
non_adverse_settle <- settle %>%
  filter(Binary_effect_limited_settle=="0") %>%
...
summarize(non_adverse_settle, NOAEL=max(Sed_level_mg_cm2_d)) #NOAEL is 1
...
adverse_settleSS <- settleSS %>% 
  filter(Binary_effect_limited_settlement=="1") %>%
  arrange(Sed_level_mg_L)

#LOAEL
summarize(adverse_settleSS, LOAEL=min(Sed_level_mg_L)) #LOAEL is 57.79 and comes from Gilmour 1999: Acropora digitifera, 2 days of exposure between 6 and 8 days post-fertilization
loaelSS1 <- adverse_settleSS %>% summarize(min(Sed_level_mg_L))
loaelSS1 <- as.numeric(loaelSS1)

#Setting up for NOAEL by filtering for non-adverse effects (binary=0), and then by any sediment level less than or equal to the LOAEL
non_adverse_settleSS <- settleSS %>%
  filter(Binary_effect_limited_settlement=="0") %>%
  arrange(desc(Sed_level_mg_L)) %>%
  filter(Sed_level_mg_L<=57.79) #less than or equal to the LOAEL, which = 57.79

#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_settleSS, NOAEL=max(Sed_level_mg_L)) #NOAEL is 34.6
...
adverse_survivalSS <- survivalSS %>% 
  filter(Binary_effect_larval_mortality=="1") %>%
  arrange(Sed_level_mg_L)

#LOAEL
summarize(adverse_survivalSS, LOAEL=min(Sed_level_mg_L)) #LOAEL is 30 and comes from Ricardo et al. (2016): Acropora tenuis, measured 2.5 days after start of experiment, when larva was 5.5 days post-fertilization
loaelSS2 <- adverse_survivalSS %>% summarize(min(Sed_level_mg_L))
loaelSS2 <- as.numeric(loaelSS2)

#Setting up for NOAEL by filtering for non-adverse effects (binary=0), and then by any sediment level less than or equal to the LOAEL
non_adverse_survivalSS <- survivalSS %>%
  filter(Binary_effect_larval_mortality=="0") %>%
  arrange(desc(Sed_level_mg_L)) %>%
  filter(Sed_level_mg_L<=30) #less than or equal to the LOAEL, which = 30

#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_survivalSS, NOAEL=max(Sed_level_mg_L)) #NOAEL is 29.5
...
survivalSS_plot2 + annotate("rect", xmin=30, xmax=Inf, 
                         ymin=-Inf, ymax=Inf, fill = "red", alpha=0.2) +
  labs(caption = "LOAEL concentration = 30.0\nNOAEL concentration = 29.5\nn = 50 datapoints, n = 4 studies from 3 articles, n = 4 spp. from 2 genera")
...
modDS1 <- glm(Binary_effect_limited_settle ~ Sed_level_mg_cm2_d, 
            family = binomial(link = "logit"), data = settleDS2)

#Models with random effect for coral species or study
modDS4 <- glmer(Binary_effect_limited_settle ~ Sed_level_mg_cm2_d + (1 | Gsp), 
              family = binomial, data = settleDS2)
modDS7 <- glmer(Binary_effect_limited_settle ~ Sed_level_mg_cm2_d + (1 | Ref), 
              family = binomial, data = settleDS2)
modDS10 <- glmer(Binary_effect_limited_settle ~ Sed_level_mg_cm2_d + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = settleDS2)
...
#proportion correct:
mean(p==settleDS2$Binary_effect_limited_settle) #[1] 0.8444444
#predicted-vs-observed table:
table(p,settleDS2$Binary_effect_limited_settle)
#ROC and area under the curve:
pred_modDS7 <- predict(modDS7, newdata = settleDS2, type = "response")
auc(roc(settleDS2$Binary_effect_limited_settle, pred_modDS7)) #AUC: 0.9167
...
modDS7_noSed <- glmer(Binary_effect_limited_settle ~ 1 + (1 | Ref), 
                    family = binomial, data = settleDS2)
...
for (i in 1:length(random.intercepts)) {
  if (i == 1) curve(exp(Ref.intercepts[i] + fixef(modDS7)["Sed_level_mg_cm2_d"]*x)/(1 + exp(Ref.intercepts[i] + fixef(modDS7)["Sed_level_mg_cm2_d"]*x)), 
                    from = min(settleDS2$Sed_level_mg_cm2_d), to = max(settleDS2$Sed_level_mg_cm2_d), 
                    ylim = c(0, 1), 
...
settleDS_plot <- ggplot(data = settleDS3) +
    ...
    scale_x_log10(limits=c(0.1,1100),breaks=c(0.01,0.1,1,10,100,1000),
                  label=c("0.01","0.1","1","10","100","1000")) +
    geom_line(aes(x = Sed_level_mg_cm2_d, y = pred_modDS7), inherit.aes=FALSE) +
    theme_bw()
...
```{r}
jvalues <- with(settleDS2, seq(from = min(Sed_level_mg_cm2_d), to = max(Sed_level_mg_cm2_d), length.out = 100))

# calculate predicted probabilities and store in a list
pp <- lapply(jvalues, function(j) {
    settleDS2$Sed_level_mg_cm2_d <- j
...
#Models with fixed effect(s) only
modSS1 <- glm(Binary_limited_settlement ~ Sed_level_mg_L, 
            family = binomial(link = "logit"), data = settleSS2)

#models with random effect for coral species or study
modSS4 <- glmer(Binary_limited_settlement ~ Sed_level_mg_L + (1 | Gsp), 
              family = binomial, data = settleSS2)
modSS7 <- glmer(Binary_limited_settlement ~ Sed_level_mg_L + (1 | Ref), 
              family = binomial, data = settleSS2)
modSS10 <- glmer(Binary_limited_settlement ~ Sed_level_mg_L + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = settleSS2)
modSS13 <- glmer(Binary_limited_settlement ~ Sed_level_mg_L + (1 | Ref_name/Ref), 
              family = binomial, data = settleSS2)
...
modSS7_log_noSed <- glmer(Binary_limited_settlement ~ 1 + (1 | Ref), 
                  family = binomial, data = settleSS2)
#Explained_var_Ref <- (1-VarCorr(modSS7_log)$Ref[1]/VarCorr(modSS7_log_noSed)$Ref[1]) #not returning a proportion of variance explained, so not sure how to interpret.
...
settleSS_plot <- ggplot(data = settleSS3) +
    ...
    scale_x_log10(limits=c(1,1000),breaks=c(0.01,0.1,1,10,100,1000),
                  label=c("0.01","0.1","1","10","100","1000")) +
    geom_line(aes(x = Sed_level_mg_L, y = pred_modSS7_log), inherit.aes=FALSE) +
    theme_bw()
...
settleSS_plot2 <- ggplot() +
    ...
    labs(x = "Sediment exposure concentration (mg/L)", 
         ...
         color = "Binary Data\nby Study",
         linetype = "Predicted\nProbability") +
    scale_x_log10(limits=c(1,max(settleSS2$Sed_level_mg_L)), 
...
#Models with fixed effect(s) only
modSSb1 <- glm(Binary_larval_mortality ~ Sed_level_mg_L, 
            family = binomial(link = "logit"), data = survivalSS2)
modSSb1b <- glm(Binary_larval_mortality ~ Sed_level_mg_L + Coral_age_at_start_expt_days, 
            family = binomial(link = "logit"), data = survivalSS2)
AIC(modSSb1,modSSb1b) #no evidence that coral age improves analysis

#models with random effect for coral species or study
modSSb4 <- glmer(Binary_larval_mortality ~ Sed_level_mg_L + (1 | Gsp), 
              family = binomial, data = survivalSS2)
modSSb7 <- glmer(Binary_larval_mortality ~ Sed_level_mg_L + (1 | Ref), 
              family = binomial, data = survivalSS2)
modSSb10 <- glmer(Binary_larval_mortality ~ Sed_level_mg_L + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = survivalSS2)
modSSb13 <- glmer(Binary_larval_mortality ~ Sed_level_mg_L + (1 | Ref_name/Ref), 
              family = binomial, data = survivalSS2)
...
#proportion correct:
mean(p==survivalSS2$Binary_larval_mortality) #[1] 0.8333333
#predicted-vs-observed table:
table(p,survivalSS2$Binary_larval_mortality)
#ROC and area under the curve:
pred_modSSb1 <- predict(modSSb1, newdata = survivalSS2, type = "response")
auc(roc(survivalSS2$Binary_larval_mortality, pred_modSSb1)) #AUC:0.6388
#can also check out 'performance' package https://cran.r-project.org/web/packages/performance/performance.pdf
...
survivalSS_plot <- ggplot(data = survivalSS3) +
    ...
    scale_x_log10(limits=c(1,1000),breaks=c(0.01,0.1,1,10,100,1000),
                  label=c("0.01","0.1","1","10","100","1000")) +
    geom_line(aes(x = Sed_level_mg_L, y = pred_modSSb1), inherit.aes=FALSE) +
    theme_bw()
...
#### By sediment exposure concentration
```{r}
jvalues <- with(survivalSS2, seq(from = min(Sed_level_mg_L), to = max(Sed_level_mg_L), length.out = 100))

# calculate predicted probabilities and store in a list
pp <- lapply(jvalues, function(j) {
    survivalSS2$Sed_level_mg_L <- j
...
survivalSS_plot2 <- ggplot() +
    ...
    labs(x = "Sediment exposure concentration (mg/L)", 
         ...
         color = "Binary Data\nby Study",
         linetype = "Predicted\nProbability") +
    scale_x_log10(limits=c(1,max(survivalSS2$Sed_level_mg_L)), 
