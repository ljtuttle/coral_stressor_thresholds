...
non_adverse <- binaryDS %>% 
  select(Ref, Ref_name, Control_code, Sed_level_mg_cm2_d, Updated_Genus, Gsp, Ocean, Sed_exposure_days, Sed_days, Binary_effect_non_adverse) %>% 
...
non_adverseSS <- binarySS %>% 
  select(Ref, Ref_name, Control_code, Sed_level_mg_L, Updated_Genus, Gsp, Ocean, Sed_exposure_days, Binary_effect_non_adverse) %>% 
  filter(!is.na(Binary_effect_non_adverse)) #remove NAs (studies that did not describe non-adverse effects)
...
#DEPOSITED SEDIMENT
adverse <- binaryDS %>% 
  select(Ref, Ref_name, Control_code, Sed_level_mg_cm2_d, Updated_Genus, Gsp, Ocean, Sed_exposure_days, Sed_days, Binary_effect_adverse, Coral_age_class) %>% 
...
#SUSPENDED SEDIMENT
adverseSS <- binarySS %>% 
  select(Ref, Ref_name, Control_code, Sed_level_mg_L, Updated_Genus, Gsp, Ocean, Sed_exposure_days, Binary_effect_adverse, Coral_age_class) %>% 
  filter(!is.na(Binary_effect_adverse)) #remove NAs (studies that did not describe adverse effects)
...
#DEPOSITED SEDIMENT
any_mort <- binaryDS %>% 
  select(Ref, Ref_name, Control_code, Sed_level_mg_cm2_d, Updated_Genus, Gsp, Ocean, Sed_exposure_days, Sed_days, Binary_sed_burial, Binary_effect_mortality) %>% 
...
#SUSPENDED SEDIMENT
any_mortSS <- binarySS %>% 
  select(Ref, Ref_name, Control_code, Sed_level_mg_L, Updated_Genus, Gsp, Ocean, Sed_exposure_days, Binary_effect_mortality) %>%
  filter(!is.na(Binary_effect_mortality)) #remove NAs (studies that did not describe total mortality)
...
##### Non-adverse effects
**Legend**: Non-adverse effects due to sedimentation? Empty circles are no (0), and filled squares are yes (1). Colors are for different genera.  
  
**Deposited sediment**: All species, n=605 datapoints, n=17 studies, n=39 genera
```{r, echo=FALSE, warning=FALSE}
#DEPOSITED SEDIMENT
non_adverse_plot2 <- ggplot(data=non_adverse) +
    ...
    theme_bw()
non_adverse_plotly <- ggplotly(non_adverse_plot2, tooltip="text")
#non_adverse_plotly #plot where I can hover over points and get species, but legend cut-off
#Error in gsub("\n", br(), a, fixed = TRUE) : input string 1 is invalid in this locale
```
But there is the following error:  
"Error in gsub("\n", br(), a, fixed = TRUE) : input string 1 is invalid in this locale"
  
**Suspended sediment**: All species, n=125 datapoints, n=4 studies, n=6 genera
```{r, echo=FALSE, warning=FALSE}
#SUSPENDED SEDIMENT
non_adverseSS_plot2 <- ggplot(data=non_adverseSS) +
    geom_point(mapping = aes(
      x = Sed_level_mg_L, 
      y = Sed_exposure_days,
      shape = factor(Binary_effect_non_adverse),
    ...
    scale_shape_manual(breaks = c("0","1"), values = c(21,15)) +
    theme_bw()
non_adverseSS_plotly <- ggplotly(non_adverseSS_plot2, tooltip="text")
non_adverseSS_plotly
```
  
  
  
##### Adverse effects 
**Legend**: Adverse effects due to sedimentation? Empty circles are no (0), and filled squares are yes (1). Colors are for different genera.
  
**Deposited sediment**: All species, n=1256 datapoints, n=40 studies, n=51 genera
```{r, echo=FALSE, warning=FALSE}
adverse_plot2 <- ggplot(data=adverse) +
    geom_point(mapping = aes(
      x = Sed_level_mg_cm2_d, 
      y = Sed_days,
      shape = factor(Binary_effect_adverse),
    ...
    scale_shape_manual(breaks = c("0","1"), values = c(21,15)) +
    theme_bw()
adverse_plotly <- ggplotly(adverse_plot2, tooltip="text")
#adverse_plotly #plot where I can hover over points and get species, but legend cut-off
```
But there is the following error:  
Error when try to plot, need to revisit: "Error in gsub("\n", br(), a, fixed = TRUE) : input string 1 is invalid in this locale"  
    
**Suspended sediment**: All species, n=585 datapoints, n=37 studies, n=18 genera
```{r, echo=FALSE, warning=FALSE}
adverseSS_plot2 <- ggplot(data=adverseSS) +
    geom_point(mapping = aes(
      x = Sed_level_mg_L, 
      y = Sed_exposure_days,
      shape = factor(Binary_effect_adverse),
    ...
    scale_shape_manual(breaks = c("0","1"), values = c(21,15)) +
    theme_bw()
adverseSS_plotly <- ggplotly(adverseSS_plot2, tooltip="text")
adverseSS_plotly #plot where I can hover over points and get species, but legend cut-off
```
  
  
##### Any mortality
**Legend**: Any mortality due to sedimentation? Empty circles are no (0), and filled squares are yes (1). Colors are for different genera.
  
**Deposited sediment**: All species, n=906 datapoints, n=31 studies, n=47 genera
```{r, echo=FALSE, warning=FALSE}
any_mort_plot2 <- ggplot(data=any_mort) +
    geom_point(mapping = aes(
      x = Sed_level_mg_cm2_d, 
      y = Sed_days,
      shape = factor(Binary_effect_mortality),
      ...
      position = "jitter",
      text = Gsp)) + 
    labs(x = "Sediment exposure concentration (mg/cm^2/day)", 
         y = "Exposure duration (days)",
         shape = "Any mortality due\nto sedimentation?",
         color = "Genus") +
    scale_x_log10(breaks=c(0.01,0.1,1,10,100,1000),
                  label=c("0.01","0.1","1","10","100","1000")) +
    scale_y_log10(limits=c(0.1,200), breaks=c(0.1,1,10,100),
    ...
    scale_shape_manual(breaks = c("0","1"), values = c(21,15)) +
    theme_bw()
any_mort_plotly <- ggplotly(any_mort_plot2, tooltip="text")
any_mort_plotly #plot where I can hover over points and get species, but legend cut-off
```
  
**Suspended sediment**: All species, n=376 datapoints, n=19 studies, n=15 genera
```{r, echo=FALSE, warning=FALSE}
any_mortSS_plot2 <- ggplot(data=any_mortSS) +
    geom_point(mapping = aes(
      x = Sed_level_mg_L, 
      y = Sed_exposure_days,
      shape = factor(Binary_effect_mortality),
    ...
    labs(x = "Sediment exposure concentration (mg/L)", 
         y = "Exposure duration (days)",
         shape = "Any mortality due to\nsuspended sediment?",
         color = "Genus") +
    scale_x_log10(limits=c(0.1,200), breaks=c(0.01,0.1,1,10,100), 
                  label=c("0.01","0.1","1","10","100")) +
    scale_y_log10(limits=c(1,300), breaks=c(0.01,0.1,1,10,100),
                  label=c("0.01","0.1","1","10","100")) +
    scale_shape_manual(breaks = c("0","1"), values = c(21,15)) +
    theme_bw()
any_mortSS_plotly <- ggplotly(any_mortSS_plot2, tooltip="text")
any_mortSS_plotly #plot where I can hover over points and get species, but legend cut-off
```  
...
```{r, echo=FALSE, warning=FALSE}
nadv_montipora <- non_adverse %>% 
  filter(Updated_Genus=="Montipora")
#nadv_montipora %>% tally() #n=207 datapoints, tally() returns same result as count(nadv_montipora)
#nadv_montipora %>% count(Ref_name, sort=T) #n=5 studies, count(Ref_name) = group_by(Ref_name) %>% tally()
#nadv_montipora %>% count(Gsp, sort=T) #n=6 species, same as nlevels(as.factor(nadv_montipora$Gsp))
  
nadv_mont_plot <- ggplot(data=nadv_montipora) +
  geom_point(mapping = aes(
      x = Sed_level_mg_cm2_d, 
      y = Sed_days,
      color = factor(Binary_effect_non_adverse),
...
adv_montipora <- adverse %>% 
  filter(Updated_Genus=="Montipora")
#adv_montipora %>% tally() #n=307 datapoints, tally() returns same result as count(nadv_montipora)
#adv_montipora %>% count(Ref_name, sort=T) #n=13 studies, count(Ref_name) = group_by(Ref_name) %>% tally()
#adv_montipora %>% count(Gsp, sort=T) #n=14 species, same as nlevels(as.factor(nadv_montipora$Gsp))

adv_mont_plot <- ggplot(data=adv_montipora) +
  geom_point(mapping = aes(
      x = Sed_level_mg_cm2_d, 
      y = Sed_days,
      color = factor(Binary_effect_adverse),
...
any_mort_montipora <- any_mort %>% 
  filter(Updated_Genus=="Montipora")
#any_mort_montipora %>% tally() #n=137 datapoints, tally() returns same result as count(nadv_montipora)
#any_mort_montipora %>% count(Ref_name, sort=T) #n=11 studies, count(Ref_name) = group_by(Ref_name) %>% tally()
#any_mort_montipora %>% count(Gsp, sort=T) #n=12 species, same as nlevels(as.factor(nadv_montipora$Gsp))

any_mort_mont_plot <- ggplot(data=any_mort_montipora) +
  geom_point(mapping = aes(
      x = Sed_level_mg_cm2_d, 
      y = Sed_days,
      color = factor(Binary_effect_mortality),
...
#PLOTS:
#nadv_mont_plot + ggtitle("Non-adverse effects, Montipora spp. only,\nn=207 datapoints, n=5 studies, n=6 spp.")
#adv_mont_plot + ggtitle("Adverse effects, Montipora spp. only,\nn=307 datapoints, n=13 studies, n=14 spp.")
#any_mort_mont_plot + ggtitle("Any mortality, Montipora spp. only,\nn=137 datapoints, n=11 studies, n=12 spp.")
```

##### Non-adverse effects, Montipora spp. only,\nn=207 datapoints, n=5 studies, n=6 spp.
**Legend**: Non-adverse effects due to sedimentation? Empty circles are no (0), and filled squares are yes (1). Colors are for different species.
```{r, echo=FALSE, warning=FALSE}
#PLOTS:
nadv_mont_plotly <- ggplotly(nadv_mont_plot, tooltip= c("x", "y", "text"))
nadv_mont_plotly
```

##### Adverse effects, Montipora spp. only,\nn=307 datapoints, n=13 studies, n=14 spp.
**Legend**: Adverse effects due to sedimentation? Empty circles are no (0), and filled squares are yes (1). Colors are for different species.
```{r, echo=FALSE, warning=FALSE}
#PLOTS:
adv_mont_plotly <- ggplotly(adv_mont_plot, tooltip= c("x", "y", "text"))
adv_mont_plotly
```

##### Any mortality, Montipora spp. only,\nn=137 datapoints, n=11 studies, n=12 spp.
**Legend**: Any mortality due to sedimentation? Empty circles are no (0), and filled squares are yes (1). Colors are for different species.
```{r, echo=FALSE, warning=FALSE}
#PLOTS:
any_mort_mont_plotly <- ggplotly(any_mort_mont_plot, tooltip= c("x", "y", "text"))
any_mort_mont_plotly
```
  
  
### *Acropora* spp. only for Suspended Sediment  
*Acropora* is the genus with the most data and appears to be among the most sensitive, so let's explore it individually:

```{r, echo=FALSE, warning=FALSE}
nadv_Acropora <- non_adverseSS %>% 
  filter(Updated_Genus=="Acropora")
#nadv_Acropora %>% tally() #n=77 datapoints, tally() returns same result as count(nadv_Acropora)
#nadv_Acropora %>% count(Ref_name, sort=T) #n=3 studies, count(Ref_name) = group_by(Ref_name) %>% tally()
#nadv_Acropora %>% count(Gsp, sort=T) #n=2 species, same as nlevels(as.factor(nadv_Acropora$Gsp))
  
nadv_acro_plot <- ggplot(data=nadv_Acropora) +
  geom_point(mapping = aes(
      x = Sed_level_mg_L, 
      y = Sed_exposure_days,
      color = factor(Binary_effect_non_adverse),
    ...
    labs(x = "Sediment exposure concentration (mg/L)", 
         y = "Exposure duration (days)",
         color = "Non-adverse effects due to\n suspended sediment?") +
    scale_x_log10(limits=c(0.1,1000), breaks=c(0.01,0.1,1,10,100,1000), label=c("0.01","0.1","1","10","100","1000")) +
    scale_y_log10(limits=c(0.1,100), breaks=c(0.01,0.1,1,10,100), 
                  label=c("0.01","0.1","1","10","100")) +
    scale_color_manual(breaks = c("0","1"), values = c("blue", "red")) +
    theme_bw()
...
adv_Acropora <- adverseSS %>% 
  filter(Updated_Genus=="Acropora")
#adv_Acropora %>% tally() #n=87 datapoints, tally() returns same result as count(nadv_Acropora)
#adv_Acropora %>% count(Ref_name, sort=T) #n=4 studies, count(Ref_name) = group_by(Ref_name) %>% tally()
#adv_Acropora %>% count(Gsp, sort=T) #n=3 species, same as nlevels(as.factor(nadv_Acropora$Gsp))

adv_acro_plot <- ggplot(data=adv_Acropora) +
  geom_point(mapping = aes(
      x = Sed_level_mg_L, 
      y = Sed_exposure_days,
      color = factor(Binary_effect_adverse),
...
any_mort_Acropora <- any_mortSS %>% 
  filter(Updated_Genus=="Acropora")
#any_mort_Acropora %>% tally() #n=37 datapoints, tally() returns same result as count(nadv_Acropora)
#any_mort_Acropora %>% count(Ref_name, sort=T) #n=3 studies, count(Ref_name) = group_by(Ref_name) %>% tally()
#any_mort_Acropora %>% count(Gsp, sort=T) #n=2 species, same as nlevels(as.factor(nadv_Acropora$Gsp))

any_mort_acro_plot <- ggplot(data=any_mort_Acropora) +
  geom_point(mapping = aes(
      x = Sed_level_mg_L, 
      y = Sed_exposure_days,
      color = factor(Binary_effect_mortality),
...
#PLOTS:
#nadv_acro_plot + labs(
#  title = "Suspended sediment, Non-adverse effects, Acropora spp. only", 
#  subtitle = "n=77 datapoints, n=3 studies, n=2 spp.")
#adv_acro_plot + labs(
#  title = "Suspended sediment, Adverse effects, Acropora spp. only", 
#  subtitle = "n=87 datapoints, n=4 studies, n=3 spp.")
#any_mort_acro_plot + labs(
#  title = "Suspended sediment, Any mortality, Acropora spp. only",
#  subtitle = "n=37 datapoints, n=3 studies, n=2 spp.")
```

##### Non-adverse effects, Acropora spp. only,\nn=77 datapoints, n=3 studies, n=2 spp.
**Legend**: Non-adverse effects due to suspended sediment? Empty circles are no (0), and filled squares are yes (1). Colors are for different species.
```{r, echo=FALSE, warning=FALSE}
#PLOTS:
nadv_acro_plotly <- ggplotly(nadv_acro_plot, tooltip= c("x", "y", "text"))
nadv_acro_plotly
```

##### Adverse effects, Acropora spp. only,\nn=87 datapoints, n=4 studies, n=3 spp.
**Legend**: Adverse effects due to suspended sediment? Empty circles are no (0), and filled squares are yes (1). Colors are for different species.
```{r, echo=FALSE, warning=FALSE}
#PLOTS:
adv_acro_plotly <- ggplotly(adv_acro_plot, tooltip= c("x", "y", "text"))
adv_acro_plotly
```

##### Any mortality, Acropora spp. only,\nn=37 datapoints, n=3 studies, n=2 spp.
**Legend**: Any mortality due to suspended sediment? Empty circles are no (0), and filled squares are yes (1). Colors are for different species.
```{r, echo=FALSE, warning=FALSE}
#PLOTS:
any_mort_acro_plotly <- ggplotly(any_mort_acro_plot, tooltip= c("x", "y", "text"))
any_mort_acro_plotly
```
...
#LOAEL
summarize(adverse_adverse, LOAEL=min(Sed_level_mg_cm2_d)) #LOAEL is 1 and comes from Sofonia (2006) PhD Chapter 4: Montipora_tuberculosa, 15 days exposure
loael <- adverse_adverse %>% summarize(min(Sed_level_mg_cm2_d))
...
non_adverse_adverse <- adverse %>%
  ...
  filter(Sed_level_mg_cm2_d<=1) #less than or equal to the LOAEL, which = 1

#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_adverse, NOAEL=max(Sed_level_mg_cm2_d)) #NOAEL is 1
...
#LOAEL
summarize(adverse_adverse2, LOAEL=min(Sed_days)) #LOAEL is 0.5 day

#Setting up for NOAEL by filtering for non-adverse effects (binary=0), and then by any sediment level less than or equal to the LOAEL
non_adverse_adverse2 <- adverse %>%
  filter(Binary_effect_adverse=="0") %>%
  filter(Sed_days<=0.5) #less than or equal to the LOAEL, which = 0.5

#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_adverse2, NOAEL=max(Sed_days)) #NOAEL is 0.5 day
...
#LOAEL
summarize(adverse_any_mort, LOAEL=min(Sed_level_mg_cm2_d)) ##LOAEL is 4.9 and comes from Sofonia (2006) PhD Chapter 4: Montipora_tuberculosa, 15 days exposure
loael2 <- adverse_any_mort %>% summarize(min(Sed_level_mg_cm2_d))
...
#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_any_mort, NOAEL=max(Sed_level_mg_cm2_d)) #NOAEL is 4.4
...
#LOAEL
summarize(adverse_any_mort2, LOAEL=min(Sed_days)) #LOAEL is 0.917 day = 22h
...
#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_any_mort2, NOAEL=max(Sed_days)) #NOAEL is 0.917 day
...
adverse_adverseSS <- adverseSS %>% 
  ...
  filter(!is.na(Sed_level_mg_L))

#LOAEL
summarize(adverse_adverseSS, LOAEL=min(Sed_level_mg_L)) #LOAEL is 3.22 and comes from Flores et al. 2012: Montipora aequituberculata, 84 days exposure
loaelSS <- adverse_adverseSS %>% summarize(min(Sed_level_mg_L))
...
#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_adverseSS, NOAEL=max(Sed_level_mg_L)) #NOAEL is 3.22
...
#LOAEL
summarize(adverseSS_any_mort, LOAEL=min(Sed_level_mg_L)) #LOAEL is 3.22 and comes from Flores et al. 2012: Montipora aequituberculata, 84 days exposure
loaelSS2 <- adverseSS_any_mort %>% summarize(min(Sed_level_mg_L))
...
#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverseSS_any_mort, NOAEL=max(Sed_level_mg_L)) #NOAEL is 3.22
...
adverseDS3 <- adverseDS3 %>% filter(!is.na(Sed_days))

#Models with fixed effect(s) only
modDS1 <- glm(Binary_effect_adverse ~ Sed_level_mg_cm2_d, 
            family = binomial(link = "logit"), data = adverseDS3)
modDS2 <- glm(Binary_effect_adverse ~ Sed_level_mg_cm2_d + Sed_days, 
            family = binomial(link = "logit"), data = adverseDS3)
modDS3 <- glm(Binary_effect_adverse ~ Sed_level_mg_cm2_d*Sed_days, 
            family = binomial(link = "logit"), data = adverseDS3)

#models with random effect for coral species or study
modDS4 <- glmer(Binary_effect_adverse ~ Sed_level_mg_cm2_d + (1 | Gsp), 
              family = binomial, data = adverseDS3)
modDS5 <- glmer(Binary_effect_adverse ~ Sed_level_mg_cm2_d + Sed_days + (1 | Gsp), 
              family = binomial, data = adverseDS3)
modDS6 <- glmer(Binary_effect_adverse ~ Sed_level_mg_cm2_d*Sed_days + (1 | Gsp), 
              family = binomial, data = adverseDS3)
modDS7 <- glmer(Binary_effect_adverse ~ Sed_level_mg_cm2_d + (1 | Ref), 
              family = binomial, data = adverseDS3)
modDS8 <- glmer(Binary_effect_adverse ~ Sed_level_mg_cm2_d + Sed_days + (1 | Ref), 
              family = binomial, data = adverseDS3)
modDS9 <- glmer(Binary_effect_adverse ~ Sed_level_mg_cm2_d*Sed_days + (1 | Ref), 
              family = binomial, data = adverseDS3)
modDS10 <- glmer(Binary_effect_adverse ~ Sed_level_mg_cm2_d + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = adverseDS3)
...
modDS12 <- glmer(Binary_effect_adverse ~ 
                Sed_level_mg_cm2_d*Sed_days + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = adverseDS3)
modDS13 <- glmer(Binary_effect_adverse ~ Sed_level_mg_cm2_d + (1 | Ref_name/Ref), 
              family = binomial, data = adverseDS3)
...
modDS11_log <- glmer(Binary_effect_adverse ~ 
                       log10_conc + Sed_days + (1 | Gsp) + (1 | Ref), 
                     family = binomial, data = adverseDS3)
...
adverseDS_plot <- ggplot(data = adverseDS3) +
    ...
    scale_x_log10(limits=c(0.01,max(adverseDS3$Sed_level_mg_cm2_d)),
...
#Models with fixed effect(s) only
modDSb1 <- glm(Binary_effect_mortality ~ Sed_level_mg_cm2_d, 
            family = binomial(link = "logit"), data = anymortDS2)
modDSb2 <- glm(Binary_effect_mortality ~ Sed_level_mg_cm2_d + Sed_days, 
            family = binomial(link = "logit"), data = anymortDS2)
modDSb3 <- glm(Binary_effect_mortality ~ Sed_level_mg_cm2_d*Sed_days, 
            family = binomial(link = "logit"), data = anymortDS2)

#models with random effect for coral species or study
modDSb4 <- glmer(Binary_effect_mortality ~ Sed_level_mg_cm2_d + (1 | Gsp), 
              family = binomial, data = anymortDS2)
modDSb5 <- glmer(Binary_effect_mortality ~ Sed_level_mg_cm2_d + Sed_days + (1 | Gsp), 
              family = binomial, data = anymortDS2)
modDSb6 <- glmer(Binary_effect_mortality ~ Sed_level_mg_cm2_d*Sed_days + (1 | Gsp), 
              family = binomial, data = anymortDS2)
modDSb7 <- glmer(Binary_effect_mortality ~ Sed_level_mg_cm2_d + (1 | Ref), 
              family = binomial, data = anymortDS2)
modDSb8 <- glmer(Binary_effect_mortality ~ Sed_level_mg_cm2_d + Sed_days + (1 | Ref), 
              family = binomial, data = anymortDS2)
modDSb9 <- glmer(Binary_effect_mortality ~ Sed_level_mg_cm2_d*Sed_days + (1 | Ref), 
              family = binomial, data = anymortDS2)
modDSb10 <- glmer(Binary_effect_mortality ~ Sed_level_mg_cm2_d + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = anymortDS2)
...
modDSb12 <- glmer(Binary_effect_mortality ~ 
                Sed_level_mg_cm2_d*Sed_days + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = anymortDS2)
modDSb13 <- glmer(Binary_effect_mortality ~ Sed_level_mg_cm2_d + (1 | Ref_name/Ref), 
              family = binomial, data = anymortDS2)
...
#Trying out a log transformation of predictor...
modDSb11_log <- glmer(Binary_effect_mortality ~ log10_conc + Sed_days + (1 | Gsp) + (1 | Ref), 
                      family = binomial, data = anymortDS2)
...
#Models with fixed effect(s) only
modSS1 <- glm(Binary_effect_adverse ~ Sed_level_mg_L, 
            family = binomial(link = "logit"), data = adverseSS3)
modSS2 <- glm(Binary_effect_adverse ~ Sed_level_mg_L + Sed_exposure_days, 
            family = binomial(link = "logit"), data = adverseSS3)
modSS3 <- glm(Binary_effect_adverse ~ Sed_level_mg_L*Sed_exposure_days, 
            family = binomial(link = "logit"), data = adverseSS3)

#models with random effect for coral species or study
modSS4 <- glmer(Binary_effect_adverse ~ Sed_level_mg_L + (1 | Gsp), 
              family = binomial, data = adverseSS3)
modSS5 <- glmer(Binary_effect_adverse ~ 
                  Sed_level_mg_L + Sed_exposure_days + (1 | Gsp), 
              family = binomial, data = adverseSS3)
modSS6 <- glmer(Binary_effect_adverse ~ 
                  Sed_level_mg_L*Sed_exposure_days + (1 | Gsp),
                family = binomial, data = adverseSS3)
modSS7 <- glmer(Binary_effect_adverse ~ Sed_level_mg_L + (1 | Ref), 
              family = binomial, data = adverseSS3)
modSS8 <- glmer(Binary_effect_adverse ~ 
                  Sed_level_mg_L + Sed_exposure_days + (1 | Ref), 
                family = binomial, data = adverseSS3)
modSS9 <- glmer(Binary_effect_adverse ~ 
                  Sed_level_mg_L*Sed_exposure_days + (1 | Ref),
                family = binomial, data = adverseSS3)
modSS10 <- glmer(Binary_effect_adverse ~ Sed_level_mg_L + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = adverseSS3)
...
modSS12 <- glmer(Binary_effect_adverse ~ 
                Sed_level_mg_L*Sed_exposure_days + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = adverseSS3)
modSS13 <- glmer(Binary_effect_adverse ~ Sed_level_mg_L + (1 | Ref_name/Ref), 
              family = binomial, data = adverseSS3)
...
modSS18_log_noSed <- glmer(Binary_effect_adverse ~ 1 + (1 | Gsp) + (1 | Ref), 
                           family = binomial, data = adverseSS3)
#Explained_var_Ref <- (1-VarCorr(modSS18_log)$Ref[1]/VarCorr(modSS18_log_noSed)$Ref[1]) #not returning a proportion of variance explained, so not sure how to interpret.
...
adverseSS_plot <- ggplot(data = adverseSS3) +
    ...
    labs(x = "Sediment exposure concentration (mg/L)", 
         ...
         color = "Study") +
    scale_x_log10(limits=c(0.1,max(adverseSS3$Sed_level_mg_L)),breaks=c(0.01,0.1,1,10,100,1000),
                  label=c("0.01","0.1","1","10","100","1000")) +
    geom_line(aes(x = Sed_level_mg_L, y = pred_modSS18_log), inherit.aes=FALSE) +
    theme_bw()
...
adverseSS_plot2 <- ggplot() +
    ...
    labs(x = "Sediment exposure concentration (mg/L)", 
         y = "Predicted probability of adverse\neffect due to sediment exposure",
         linetype = "Predicted\nProbability") +
    scale_x_log10(limits=c(0.1,max(adverseSS3$Sed_level_mg_L)), 
...
#Models with fixed effect(s) only
modSS1 <- glm(Binary_effect_mortality ~ Sed_level_mg_L, 
            family = binomial(link = "logit"), data = anymortSS2)
modSS2 <- glm(Binary_effect_mortality ~ Sed_level_mg_L + Sed_exposure_days, 
            family = binomial(link = "logit"), data = anymortSS2)
modSS3 <- glm(Binary_effect_mortality ~ Sed_level_mg_L*Sed_exposure_days, 
            family = binomial(link = "logit"), data = anymortSS2)

#models with random effect for coral species or study
modSS4 <- glmer(Binary_effect_mortality ~ Sed_level_mg_L + (1 | Gsp), 
...
modSS6 <- glmer(Binary_effect_mortality ~ 
                  Sed_level_mg_L*Sed_exposure_days + (1 | Gsp),
                family = binomial, data = anymortSS2)
modSS7 <- glmer(Binary_effect_mortality ~ Sed_level_mg_L + (1 | Ref), 
              family = binomial, data = anymortSS2)
...
modSS9 <- glmer(Binary_effect_mortality ~ 
                  Sed_level_mg_L*Sed_exposure_days + (1 | Ref),
                family = binomial, data = anymortSS2)
modSS10 <- glmer(Binary_effect_mortality ~ Sed_level_mg_L + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = anymortSS2)
...
modSS12 <- glmer(Binary_effect_mortality ~ 
                Sed_level_mg_L*Sed_exposure_days + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = anymortSS2)
modSS13 <- glmer(Binary_effect_mortality ~ Sed_level_mg_L + (1 | Ref_name/Ref), 
              family = binomial, data = anymortSS2)
...
modSSb18_log_noSed <- glmer(Binary_effect_mortality ~ 1 + (1 | Gsp), 
                      family = binomial, data = anymortSS2)
#Explained_var_Ref <- (1-VarCorr(modSSb18_log)$Ref[1]/VarCorr(modSSb18_log_noSed)$Ref[1]) #not returning a proportion of variance explained, so not sure how to interpret.
random.intercepts <- ranef(modSSb18_log)$Ref[["(Intercept)"]]
Ref.intercepts <- fixef(modSSb18_log)[1] + random.intercepts 
#plot with probability of binary response on y-axis, each line representing a study:
for (i in 1:length(random.intercepts)) {
  if (i == 1) curve(exp(Ref.intercepts[i] + fixef(modSSb18_log)["Sed_level_mg_L"]*x)/(1 + exp(Ref.intercepts[i] + fixef(modSSb18_log)["Sed_level_mg_L"]*x)), 
                    from = min(anymortSS2$Sed_level_mg_L), to = max(anymortSS2$Sed_level_mg_L), 
                    ylim = c(0, 1), 
                    ylab = "Probability of adverse effect", 
                    xlab = "log10(exposure concentration, mg/cm2/d)")
  if (i > 1) curve(exp(Ref.intercepts[i] + fixef(modSSb18_log)["Sed_level_mg_L"]*x)/(1 + exp(Ref.intercepts[i] + fixef(modSSb18_log)["Sed_level_mg_L"]*x)), add = T) 
}
...
anymortSS_plot <- ggplot(data = anymortSS3) +
    ...
    labs(x = "Sediment exposure concentration (mg/L)", 
         ...
         color = "Study") +
    scale_x_log10(limits=c(0.1,max(anymortSS3$Sed_level_mg_L)),breaks=c(0.01,0.1,1,10,100,1000),
                  label=c("0.01","0.1","1","10","100","1000")) +
    geom_line(aes(x = Sed_level_mg_L, y = pred_modSSb18_log), inherit.aes=FALSE) +
    theme_bw()
...
anymortSS_plot2 <- ggplot() +
    ...
    labs(x = "Sediment exposure concentration (mg/L)", 
         ...
         linetype = "Predicted\nProbability") +
    scale_x_log10(limits=c(0.1,max(anymortSS2$Sed_level_mg_L)), 
