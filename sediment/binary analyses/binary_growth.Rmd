---
title: "Binary effects of sediment on the growth of adult corals"
...
growthDS <- binary %>%
  ...
  select(Ref, Ref_name, Control_code, Sed_level_stand_mg, Binary_reduced_growth, Updated_Genus, Gsp, Ocean, Sed_exposure_days)
...
growthDS_plot <- ggplot(data=growthDS, mapping = aes(
      ...
      position = "jitter",
      text = Gsp)) +
    geom_point(data = subset(growthDS, Binary_reduced_growth=="0")) + 
...
#PLOTS:
growthDS_plot + geom_point(data = subset(growthDS, Binary_reduced_growth=="1")) +
...
adverse_growthDS <- growthDS %>% 
  filter(Binary_reduced_growth=="1") %>%
  arrange(Sed_level_stand_mg) #minimum Sedimentation level is 53 but only two rows from Lirman et al. (2008)!!

#LOAEL
summarize(adverse_growthDS, LOAEL=min(Sed_level_stand_mg)) #LOAEL is 53 and comes from Lirman et al. (2008)!!
loael <- adverse_growthDS %>% summarize(min(Sed_level_stand_mg))
...
non_adverse_growthDS <- growthDS %>%
  filter(Binary_reduced_growth=="0") %>%
  arrange(desc(Sed_level_stand_mg)) %>%
  filter(Sed_level_stand_mg<=53) #less than or equal to the LOAEL, which = 53

#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_growthDS, NOAEL=max(Sed_level_stand_mg)) #NOAEL is 38.4

#PLOT
ggplot(data=growthDS) +
...
```{r, echo=FALSE, warning=FALSE}
#Setting up for LOAEL by filtering for adverse effects (binary=1)
adverse_growthDS2 <- growthDS %>% 
  filter(Binary_reduced_growth=="1") %>% 
  filter(!is.na(Sed_exposure_days)) %>% 
  arrange(Sed_exposure_days)

#LOAEL
summarize(adverse_growthDS2, LOAEL=min(Sed_exposure_days)) #LOAEL is 21 days

#Setting up for NOAEL by filtering for non-adverse effects (binary=0), and then by any sediment level less than or equal to the LOAEL
non_adverse_growthDS2 <- growthDS %>%
  filter(Binary_reduced_growth=="0") %>%
  filter(Sed_exposure_days<=21) #less than or equal to the LOAEL, which = 21

#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_growthDS2, NOAEL=max(Sed_exposure_days)) #NOAEL is 21 days
...
#LOAEL
summarize(adverse_growthSS, LOAEL=min(Sed_level_stand_mg)) #LOAEL is 58.6 and comes from Te (2001 Chapter 6) with Montipora_verrucosa for 31 days exposure
loaelSS <- adverse_growthSS %>% summarize(min(Sed_level_stand_mg))
...
non_adverse_growthSS <- growthSS %>%
  filter(Binary_reduced_growth=="0") %>%
  arrange(desc(Sed_level_stand_mg)) %>%
  filter(Sed_level_stand_mg<=58.6) #less than or equal to the LOAEL, which = 58.6

#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_growthSS, NOAEL=max(Sed_level_stand_mg)) #NOAEL is 49

#PLOT
ggplot(data = growthSS) +
    geom_point(mapping = aes(
      x = Sed_level_stand_mg,
      y = Binary_reduced_growth)) + 
...
mod1 <- glm(Binary_reduced_growth ~ Sed_level_stand_mg, 
            family = binomial(link = "logit"), data = growthDS2)
mod2 <- glm(Binary_reduced_growth ~ Sed_level_stand_mg + Sed_exposure_days, 
            family = binomial(link = "logit"), data = growthDS2)
mod3 <- glm(Binary_reduced_growth ~ Sed_level_stand_mg*Sed_exposure_days, 
            family = binomial(link = "logit"), data = growthDS2)

#Models with random effect for coral species or study
mod4 <- glmer(Binary_reduced_growth ~ Sed_level_stand_mg + (1 | Gsp), 
              family = binomial, data = growthDS2)
mod5 <- glmer(Binary_reduced_growth ~ Sed_level_stand_mg + Sed_exposure_days + (1 | Gsp), 
              family = binomial, data = growthDS2)
mod6 <- glmer(Binary_reduced_growth ~ Sed_level_stand_mg*Sed_exposure_days + (1 | Gsp), 
              family = binomial, data = growthDS2)
mod7 <- glmer(Binary_reduced_growth ~ Sed_level_stand_mg + (1 | Ref), 
              family = binomial, data = growthDS2)
mod8 <- glmer(Binary_reduced_growth ~ Sed_level_stand_mg + Sed_exposure_days + (1 | Ref), 
              family = binomial, data = growthDS2)
mod9 <- glmer(Binary_reduced_growth ~ Sed_level_stand_mg*Sed_exposure_days + (1 | Ref), 
              family = binomial, data = growthDS2)
mod10 <- glmer(Binary_reduced_growth ~ Sed_level_stand_mg + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = growthDS2)
...
mod12 <- glmer(Binary_reduced_growth ~ 
                Sed_level_stand_mg*Sed_exposure_days + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = growthDS2)
mod13 <- glmer(Binary_reduced_growth ~ Sed_level_stand_mg + (1 | Ref_name/Ref), 
              family = binomial, data = growthDS2)
...
for (i in 1:length(random.intercepts)) {
  if (i == 1) curve(exp(Ref.intercepts[i] + fixef(mod7)["Sed_level_stand_mg"]*x)/(1 + exp(Ref.intercepts[i] + fixef(mod7)["Sed_level_stand_mg"]*x)), 
                    from = min(growthDS2$Sed_level_stand_mg), to = max(growthDS2$Sed_level_stand_mg), 
                    ylim = c(0, 1), 
                    ylab = "Probability of adverse effect", 
                    xlab = "Exposure concentration, mg/cm2/d")
  if (i > 1) curve(exp(Ref.intercepts[i] + fixef(mod7)["Sed_level_stand_mg"]*x)/(1 + exp(Ref.intercepts[i] + fixef(mod7)["Sed_level_stand_mg"]*x)), add = T) 
}
...
#Sed_level_stand_mg 1.001081 0.99498828 1.007212
...
growthDS_plot <- ggplot(data = growthDS3) +
    ...
    scale_x_log10(limits=c(0.01,1000),breaks=c(0.01,0.1,1,10,100,1000),
                  label=c("0.01","0.1","1","10","100","1000")) +
    geom_line(aes(x = Sed_level_stand_mg, y = pred_mod7), inherit.aes=FALSE) +
    theme_bw()
...
```{r}
#summary(growthDS2$Sed_level_stand_mg)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#   0.02   35.95  126.70  134.49  200.00  426.80

jvalues <- with(growthDS2, seq(from = min(Sed_level_stand_mg), to = max(Sed_level_stand_mg), length.out = 100))

# calculate predicted probabilities and store in a list
pp <- lapply(jvalues, function(j) {
    growthDS2$Sed_level_stand_mg <- j
...
#Models with fixed effect(s) only
modSS1 <- glm(Binary_reduced_growth ~ Sed_level_stand_mg, 
            family = binomial(link = "logit"), data = growthSS2)
modSS2 <- glm(Binary_reduced_growth ~ Sed_level_stand_mg + Sed_exposure_days, 
            family = binomial(link = "logit"), data = growthSS2)
modSS3 <- glm(Binary_reduced_growth ~ Sed_level_stand_mg*Sed_exposure_days, 
            family = binomial(link = "logit"), data = growthSS2)

#modSSels with random effect for coral species or study
modSS4 <- glmer(Binary_reduced_growth ~ Sed_level_stand_mg + (1 | Gsp), 
              family = binomial, data = growthSS2)
modSS5 <- glmer(Binary_reduced_growth ~ Sed_level_stand_mg + Sed_exposure_days + (1 | Gsp), 
              family = binomial, data = growthSS2)
#modSS6 <- glmer(Binary_reduced_growth ~ Sed_level_stand_mg*Sed_exposure_days + (1 | Gsp), 
#              family = binomial, data = growthSS2)
...
modSS7 <- glmer(Binary_reduced_growth ~ Sed_level_stand_mg + (1 | Ref), 
              family = binomial, data = growthSS2)
modSS8 <- glmer(Binary_reduced_growth ~ Sed_level_stand_mg + Sed_exposure_days + (1 | Ref), 
              family = binomial, data = growthSS2)
#modSS9 <- glmer(Binary_reduced_growth ~ Sed_level_stand_mg*Sed_exposure_days + (1 | Ref), 
#              family = binomial, data = growthSS2)
#Error in pwrssUpdate(pp, resp, tol = tolPwrss, GQmat = GQmat, compDev = compDev, : (maxstephalfit) PIRLS step-halvings failed to reduce deviance in pwrssUpdate
modSS10 <- glmer(Binary_reduced_growth ~ Sed_level_stand_mg + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = growthSS2)
...
#              family = binomial, data = growthSS2)
#Error in pwrssUpdate(pp, resp, tol = tolPwrss, GQmat = GQmat, compDev = compDev, : (maxstephalfit) PIRLS step-halvings failed to reduce deviance in pwrssUpdate
modSS13 <- glmer(Binary_reduced_growth ~ Sed_level_stand_mg + (1 | Ref_name/Ref), 
              family = binomial, data = growthSS2)
...
#plot with probability of binary response on y-axis, each line representing a study:
for (i in 1:length(random.intercepts)) {
  if (i == 1) curve(exp(Ref.intercepts[i] + fixef(modSS8)["Sed_level_stand_mg"]*x)/(1 + exp(Ref.intercepts[i] + fixef(modSS8)["Sed_level_stand_mg"]*x)), 
                    from = min(growthSS2$Sed_level_stand_mg), to = max(growthSS2$Sed_level_stand_mg), 
                    ylim = c(0, 1), 
                    ylab = "Probability of adverse effect", 
                    xlab = "Exposure concentration, mg/L)")
  if (i > 1) curve(exp(Ref.intercepts[i] + fixef(modSS8)["Sed_level_stand_mg"]*x)/(1 + exp(Ref.intercepts[i] + fixef(modSS8)["Sed_level_stand_mg"]*x)), add = T) 
}
...
growthSS_plot <- ggplot(data = growthSS3) +
    ...
    scale_x_log10(limits=c(1,1000),breaks=c(0.01,0.1,1,10,100,1000),
                  label=c("0.01","0.1","1","10","100","1000")) +
    geom_line(aes(x = Sed_level_stand_mg, y = pred_modSS8), inherit.aes=FALSE) +
    theme_bw()
...
```{r}
#summary(growthSS2$Sed_level_stand_mg)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#   1.31   15.79   49.00   78.83  165.00  199.00 

jvalues <- with(growthSS2, seq(from = min(Sed_level_stand_mg), to = max(Sed_level_stand_mg), length.out = 100))

# calculate predicted probabilities and store in a list
pp <- lapply(jvalues, function(j) {
    growthSS2$Sed_level_stand_mg <- j
...
growthSS_plot2 <- ggplot() +
    ...
    labs(x = "Sediment exposure concentration (mg/L)", 
         ...
         color = "Binary Data\nby Study",
         linetype = "Predicted\nProbability") +
    scale_x_log10(limits=c(1,max(growthSS2$Sed_level_stand_mg)),
