---
title: "Binary effects of sediment on the mortality of adult corals"
...
library(lattice)
library(MuMIn)
binary <- read.csv("/Volumes/GoogleDrive/My Drive/iScience/Code/coral_stressor_thresholds/sediment/data/binary_ALL.csv", header=T)
...
mortalityDS <- binary %>%
  ...
  select(Ref, Ref_name, Control_code, Sed_level_stand_mg, Binary_small_necroses, Binary_large_necroses, Binary_death_total, Updated_Genus, Gsp, Ocean, Sed_exposure_days)
  
mortalityDS %>% tally() #n=827 datapoints, tally() returns same result as count(mortalityDS)
mortalityDS %>% count(Ref, sort=T) #n=28 studies
mortalityDS %>% count(Ref_name, sort=T) #n=27 studies, count(Ref_name) = group_by(Ref_name) %>% tally()
mortalityDS %>% count(Ocean, sort=T) #n=3 oceans
mortalityDS %>% count(Gsp) %>% #n=87 species
  top_n(10)
mortalityDS %>% count(Updated_Genus, sort=T) %>% #n=46 genera
  top_n(10)
```
...
#smallDS %>% count(Ref_name, sort=T) #n=20 studies, count(Ref_name) = group_by(Ref_name) %>% tally()
...
summarize(non_adverse_small, NOAEL=max(Sed_level_stand_mg)) #NOAEL is 4.4

#PLOT
ggplot(data = mortalityDS) +
...
#LOAEL
summarize(adverse_small2, LOAEL=min(Sed_exposure_days)) #LOAEL is 0.917 days = 22h
...
#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_small2, NOAEL=max(Sed_exposure_days)) #NOAEL is 0.917 days
...
#LOAEL
summarize(adverse_large, LOAEL=min(Sed_level_stand_mg)) #LOAEL is 20.8 and comes from Hodgson (1989b) Chapter IV - field component.
loaelDS2 <- adverse_large %>% summarize(min(Sed_level_stand_mg))
...
non_adverse_large <- largeDS %>%
  ...
  arrange(desc(Sed_level_stand_mg)) %>%
  filter(Sed_level_stand_mg<=20.8) #less than or equal to the LOAEL, which = 20.8

#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_large, NOAEL=max(Sed_level_stand_mg)) #NOAEL is 20.8
...
#LOAEL
summarize(adverse_large2, LOAEL=min(Sed_exposure_days)) #LOAEL is 3 days
...
#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_large2, NOAEL=max(Sed_exposure_days)) #NOAEL is 3 days
...
#LOAEL
summarize(adverse_total, LOAEL=min(Sed_level_stand_mg)) #LOAEL is 20.8 and comes from Hodgson (1989b) Chapter IV - field component.
loaelDS3 <- adverse_total %>% summarize(min(Sed_level_stand_mg))
...
#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_total, NOAEL=max(Sed_level_stand_mg)) #NOAEL is 20.8
...
#LOAEL
summarize(adverse_total2, LOAEL=min(Sed_exposure_days)) #LOAEL is 1 day
...
#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_total2, NOAEL=max(Sed_exposure_days)) #NOAEL is 1 day
...
#LOAEL
summarize(adverse_smallSS, LOAEL=min(Sed_level_stand_mg)) #LOAEL is 3.22 and is from Flores et al. (2012) with Montipora aequituberculata for 84 days
loaelSS1 <- adverse_smallSS %>% summarize(min(Sed_level_stand_mg))
...
#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_smallSS, NOAEL=max(Sed_level_stand_mg)) #NOAEL is 3.22

#PLOT
ggplot(data = mortalitySS) +
    geom_point(mapping = aes(
      x = Sed_level_stand_mg, 
      y = Binary_small_necroses)) + 
...
#LOAEL
summarize(adverse_largeSS, LOAEL=min(Sed_level_stand_mg)) #LOAEL is 29.1 and is from Flores et al. (2012) with Montipora aequituberculata for 84 days
loaelSS2 <- adverse_largeSS %>% summarize(min(Sed_level_stand_mg))
...
non_adverse_largeSS <- largeSS %>%
  filter(Binary_large_necroses=="0") %>%
  arrange(desc(Sed_level_stand_mg)) %>%
  filter(Sed_level_stand_mg<=29.1) #less than or equal to the LOAEL, which = 29.1

#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_largeSS, NOAEL=max(Sed_level_stand_mg)) #NOAEL is 29.1
...
#LOAEL
summarize(adverse_totalSS, LOAEL=min(Sed_level_stand_mg)) #LOAEL is 29.1 and is from Flores et al. (2012) with Montipora_aequituberculata for 84 days
loaelSS3 <- adverse_totalSS %>% summarize(min(Sed_level_stand_mg))
...
#NOAEL = maximum sediment level with no adverse effect
summarize(non_adverse_totalSS, NOAEL=max(Sed_level_stand_mg)) #NOAEL is 29.1
...
modDS1 <- glm(Binary_small_necroses ~ Sed_level_stand_mg, 
            family = binomial(link = "logit"), data = smallDS2)
modDS2 <- glm(Binary_small_necroses ~ Sed_level_stand_mg + Sed_exposure_days, 
            family = binomial(link = "logit"), data = smallDS2)
modDS3 <- glm(Binary_small_necroses ~ Sed_level_stand_mg*Sed_exposure_days, 
            family = binomial(link = "logit"), data = smallDS2)

#models with random effect for coral species or study
modDS4 <- glmer(Binary_small_necroses ~ Sed_level_stand_mg + (1 | Gsp), 
              family = binomial, data = smallDS2)
modDS5 <- glmer(Binary_small_necroses ~ Sed_level_stand_mg + Sed_exposure_days + (1 | Gsp), 
              family = binomial, data = smallDS2)
modDS6 <- glmer(Binary_small_necroses ~ Sed_level_stand_mg*Sed_exposure_days + (1 | Gsp), 
              family = binomial, data = smallDS2)
modDS7 <- glmer(Binary_small_necroses ~ Sed_level_stand_mg + (1 | Ref), 
              family = binomial, data = smallDS2)
modDS8 <- glmer(Binary_small_necroses ~ Sed_level_stand_mg + Sed_exposure_days + (1 | Ref), 
              family = binomial, data = smallDS2)
modDS9 <- glmer(Binary_small_necroses ~ Sed_level_stand_mg*Sed_exposure_days + (1 | Ref), 
              family = binomial, data = smallDS2)
modDS10 <- glmer(Binary_small_necroses ~ Sed_level_stand_mg + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = smallDS2)
...
#Models with fixed effect(s) only
modDSb1 <- glm(Binary_large_necroses ~ Sed_level_stand_mg, 
            family = binomial(link = "logit"), data = largeDS2)
modDSb2 <- glm(Binary_large_necroses ~ Sed_level_stand_mg + Sed_exposure_days, 
            family = binomial(link = "logit"), data = largeDS2)
modDSb3 <- glm(Binary_large_necroses ~ Sed_level_stand_mg*Sed_exposure_days, 
            family = binomial(link = "logit"), data = largeDS2)

#models with random effect for coral species or study
modDSb4 <- glmer(Binary_large_necroses ~ Sed_level_stand_mg + (1 | Gsp), 
              family = binomial, data = largeDS2)
modDSb5 <- glmer(Binary_large_necroses ~ Sed_level_stand_mg + Sed_exposure_days + (1 | Gsp), 
              family = binomial, data = largeDS2)
modDSb6 <- glmer(Binary_large_necroses ~ Sed_level_stand_mg*Sed_exposure_days + (1 | Gsp), 
              family = binomial, data = largeDS2)
modDSb7 <- glmer(Binary_large_necroses ~ Sed_level_stand_mg + (1 | Ref), 
              family = binomial, data = largeDS2)
modDSb8 <- glmer(Binary_large_necroses ~ Sed_level_stand_mg + Sed_exposure_days + (1 | Ref), 
              family = binomial, data = largeDS2)
modDSb9 <- glmer(Binary_large_necroses ~ Sed_level_stand_mg*Sed_exposure_days + (1 | Ref), 
              family = binomial, data = largeDS2)
modDSb10 <- glmer(Binary_large_necroses ~ Sed_level_stand_mg + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = largeDS2)
modDSb11 <- glmer(Binary_large_necroses ~ 
                Sed_level_stand_mg + Sed_exposure_days + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = largeDS2)
modDSb12 <- glmer(Binary_large_necroses ~ 
                Sed_level_stand_mg*Sed_exposure_days + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = largeDS2)
...
#Trying out a log transformation of predictor...
modDSb11_log <- glmer(Binary_large_necroses ~ 
                        log10_conc + log10_dur + (1 | Gsp) + (1 | Ref), 
                     family = binomial, data = largeDS2)
...
#proportion correct:
mean(p==largeDS2$Binary_large_necroses) #[1] 0.9099617
#predicted-vs-observed table:
table(p,largeDS2$Binary_large_necroses)
#ROC and area under the curve:
pred_modDSb11_log <- predict(modDSb11_log, newdata = largeDS2, type = "response")
auc(roc(largeDS2$Binary_large_necroses, pred_modDSb11_log)) #AUC: 0.9494
#can also check out 'performance' package https://cran.r-project.org/web/packages/performance/performance.pdf
...
modDSb11_log_noSed <- glmer(Binary_large_necroses ~ 1 + (1 | Gsp) + (1 | Ref), 
                            family = binomial, data = largeDS2)
...
#Models with fixed effect(s) only
modDSc1 <- glm(Binary_death_total ~ Sed_level_stand_mg, 
            family = binomial(link = "logit"), data = totalDS2)
modDSc2 <- glm(Binary_death_total ~ Sed_level_stand_mg + Sed_exposure_days, 
            family = binomial(link = "logit"), data = totalDS2)
modDSc3 <- glm(Binary_death_total ~ Sed_level_stand_mg*Sed_exposure_days, 
            family = binomial(link = "logit"), data = totalDS2)

#models with random effect for coral species or study
modDSc4 <- glmer(Binary_death_total ~ Sed_level_stand_mg + (1 | Gsp), 
              family = binomial, data = totalDS2)
modDSc5 <- glmer(Binary_death_total ~ Sed_level_stand_mg + Sed_exposure_days + (1 | Gsp), 
              family = binomial, data = totalDS2)
modDSc6 <- glmer(Binary_death_total ~ Sed_level_stand_mg*Sed_exposure_days + (1 | Gsp), 
              family = binomial, data = totalDS2)
modDSc7 <- glmer(Binary_death_total ~ Sed_level_stand_mg + (1 | Ref), 
              family = binomial, data = totalDS2)
modDSc8 <- glmer(Binary_death_total ~ Sed_level_stand_mg + Sed_exposure_days + (1 | Ref), 
              family = binomial, data = totalDS2)
modDSc9 <- glmer(Binary_death_total ~ Sed_level_stand_mg*Sed_exposure_days + (1 | Ref), 
              family = binomial, data = totalDS2)
modDSc10 <- glmer(Binary_death_total ~ Sed_level_stand_mg + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = totalDS2)
...
#Models with fixed effect(s) only
modSS1 <- glm(Binary_small_necroses ~ Sed_level_stand_mg, 
            family = binomial(link = "logit"), data = smallSS2)
modSS2 <- glm(Binary_small_necroses ~ Sed_level_stand_mg + Sed_exposure_days, 
            family = binomial(link = "logit"), data = smallSS2)
modSS3 <- glm(Binary_small_necroses ~ Sed_level_stand_mg*Sed_exposure_days, 
            family = binomial(link = "logit"), data = smallSS2)

#models with random effect for coral species or study
modSS4 <- glmer(Binary_small_necroses ~ Sed_level_stand_mg + (1 | Gsp), 
              family = binomial, data = smallSS2)
modSS5 <- glmer(Binary_small_necroses ~ Sed_level_stand_mg + Sed_exposure_days + (1 | Gsp), 
              family = binomial, data = smallSS2)
modSS6 <- glmer(Binary_small_necroses ~ Sed_level_stand_mg*Sed_exposure_days + (1 | Gsp), 
              family = binomial, data = smallSS2)
modSS7 <- glmer(Binary_small_necroses ~ Sed_level_stand_mg + (1 | Ref), 
              family = binomial, data = smallSS2)
modSS8 <- glmer(Binary_small_necroses ~ Sed_level_stand_mg + Sed_exposure_days + (1 | Ref), 
              family = binomial, data = smallSS2)
modSS9 <- glmer(Binary_small_necroses ~ Sed_level_stand_mg*Sed_exposure_days + (1 | Ref), 
              family = binomial, data = smallSS2)
modSS10 <- glmer(Binary_small_necroses ~ Sed_level_stand_mg + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = smallSS2)
...
modSS12 <- glmer(Binary_small_necroses ~ 
                Sed_level_stand_mg*Sed_exposure_days + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = smallSS2)
modSS13 <- glmer(Binary_small_necroses ~ Sed_level_stand_mg + (1 | Ref_name/Ref), 
              family = binomial, data = smallSS2)
...
qqline(ranef(modSS5)$Gsp[,1])
VarCorr(modSS5)
exp(sqrt(VarCorr(modSS5)$Gsp[1]))
...
for (i in 1:length(random.intercepts)) {
  if (i == 1) curve(exp(Gsp.intercepts[i] + fixef(modSS5)["Sed_level_stand_mg"]*x)/(1 + exp(Gsp.intercepts[i] + fixef(modSS5)["Sed_level_stand_mg"]*x)), 
                    from = min(smallSS2$Sed_level_stand_mg), to = max(smallSS2$Sed_level_stand_mg), 
                    ylim = c(0, 1), 
                    ylab = "Probability of adverse effect", 
...
#Models with fixed effect(s) only
modSSb1 <- glm(Binary_large_necroses ~ Sed_level_stand_mg, 
            family = binomial(link = "logit"), data = largeSS2)
modSSb2 <- glm(Binary_large_necroses ~ Sed_level_stand_mg + Sed_exposure_days, 
            family = binomial(link = "logit"), data = largeSS2)
modSSb3 <- glm(Binary_large_necroses ~ Sed_level_stand_mg*Sed_exposure_days, 
            family = binomial(link = "logit"), data = largeSS2)

#models with random effect for coral species or study
modSSb4 <- glmer(Binary_large_necroses ~ Sed_level_stand_mg + (1 | Gsp), 
              family = binomial, data = largeSS2)
#modSSb5 <- glmer(Binary_large_necroses ~ Sed_level_stand_mg + Sed_exposure_days + (1 | Gsp), 
#              family = binomial, data = largeSS2)
#modSSb6 <- glmer(Binary_large_necroses ~ Sed_level_stand_mg*Sed_exposure_days + (1 | Gsp), 
#              family = binomial, data = largeSS2)
...
modSSb7 <- glmer(Binary_large_necroses ~ Sed_level_stand_mg + (1 | Ref), 
              family = binomial, data = largeSS2)
modSSb8 <- glmer(Binary_large_necroses ~ Sed_level_stand_mg + Sed_exposure_days + (1 | Ref), 
              family = binomial, data = largeSS2)
modSSb9 <- glmer(Binary_large_necroses ~ Sed_level_stand_mg*Sed_exposure_days + (1 | Ref), 
              family = binomial, data = largeSS2)
modSSb10 <- glmer(Binary_large_necroses ~ Sed_level_stand_mg + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = largeSS2)
#modSSb11 <- glmer(Binary_large_necroses ~ 
#                Sed_level_stand_mg + Sed_exposure_days + (1 | Gsp) + (1 | Ref), 
#              family = binomial, data = largeSS2)
#modSSb12 <- glmer(Binary_large_necroses ~ 
#                Sed_level_stand_mg*Sed_exposure_days + (1 | Gsp) + (1 | Ref), 
#              family = binomial, data = largeSS2)
#Error in pwrssUpdate(pp, resp, tol = tolPwrss, GQmat = GQmat, compDev = compDev, : (maxstephalfit) PIRLS step-halvings failed to reduce deviance in pwrssUpdate
modSSb13 <- glmer(Binary_large_necroses ~ Sed_level_stand_mg + (1 | Ref_name/Ref), 
              family = binomial, data = largeSS2)
modSSb14 <- glmer(Binary_large_necroses ~ 
                 Sed_level_stand_mg + Sed_exposure_days + (1 | Ref_name/Ref), 
              family = binomial, data = largeSS2)
modSSb15 <- glmer(Binary_large_necroses ~ 
                 Sed_level_stand_mg*Sed_exposure_days + (1 | Ref_name/Ref), 
              family = binomial, data = largeSS2)
modSSb16 <- glmer(Binary_large_necroses ~ 
                 Sed_level_stand_mg + (1 | Gsp) + (1 | Ref_name), 
              family = binomial, data = largeSS2)
#modSSb17 <- glmer(Binary_large_necroses ~ 
#                 Sed_level_stand_mg + Sed_exposure_days + (1 | Gsp) + (1 | Ref_name), 
#              family = binomial, data = largeSS2)
#modSSb18 <- glmer(Binary_large_necroses ~ 
#                 Sed_level_stand_mg*Sed_exposure_days + (1 | Gsp) + (1 | Ref_name), 
#              family = binomial, data = largeSS2)
#Error in pwrssUpdate(pp, resp, tol = tolPwrss, GQmat = GQmat, compDev = compDev, : (maxstephalfit) PIRLS #step-halvings failed to reduce deviance in pwrssUpdate
modSSb19 <- glmer(Binary_large_necroses ~ 
                 Sed_level_stand_mg + (1 | Gsp) + (1 | Ref_name/Ref), 
              family = binomial, data = largeSS2)
#modSSb20 <- glmer(Binary_large_necroses ~ 
#                 Sed_level_stand_mg + Sed_exposure_days + (1 | Gsp) + (1 | Ref_name/Ref), 
#              family = binomial, data = largeSS2)
#modSSb21 <- glmer(Binary_large_necroses ~ 
#                 Sed_level_stand_mg*Sed_exposure_days + (1 | Gsp) + (1 | Ref_name/Ref), 
#              family = binomial, data = largeSS2)
#Error in pwrssUpdate(pp, resp, tol = tolPwrss, GQmat = GQmat, compDev = compDev, : (maxstephalfit) PIRLS step-halvings failed to reduce deviance in pwrssUpdate
...
#Trying out a log transformation of predictor...
modSSb8_log <- glmer(Binary_large_necroses ~ 
...
#proportion correct:
mean(p==largeSS2$Binary_large_necroses) #[1] 0.9863946
#predicted-vs-observed table:
table(p,largeSS2$Binary_large_necroses)
#ROC and area under the curve:
pred_modSSb2 <- predict(modSSb2, newdata = largeSS2, type = "response")
auc(roc(largeSS2$Binary_large_necroses, pred_modSSb2)) #AUC: 0.9976
#can also check out 'performance' package https://cran.r-project.org/web/packages/performance/performance.pdf
...
#Models with fixed effect(s) only
modSSc1 <- glm(Binary_death_total ~ Sed_level_stand_mg, 
            family = binomial(link = "logit"), data = totalSS2)
modSSc2 <- glm(Binary_death_total ~ Sed_level_stand_mg + Sed_exposure_days, 
            family = binomial(link = "logit"), data = totalSS2)
modSSc3 <- glm(Binary_death_total ~ Sed_level_stand_mg*Sed_exposure_days, 
            family = binomial(link = "logit"), data = totalSS2)

#models with random effect for coral species or study
modSSc4 <- glmer(Binary_death_total ~ Sed_level_stand_mg + (1 | Gsp), 
              family = binomial, data = totalSS2)
#modSSc5 <- glmer(Binary_death_total ~ Sed_level_stand_mg + Sed_exposure_days + (1 | Gsp), 
#              family = binomial, data = totalSS2)
#modSSc6 <- glmer(Binary_death_total ~ Sed_level_stand_mg*Sed_exposure_days + (1 | Gsp), 
#              family = binomial, data = totalSS2)
#Error in pwrssUpdate(pp, resp, tol = tolPwrss, GQmat = GQmat, compDev = compDev, : (maxstephalfit) PIRLS step-halvings failed to reduce deviance in pwrssUpdate
modSSc7 <- glmer(Binary_death_total ~ Sed_level_stand_mg + (1 | Ref), 
              family = binomial, data = totalSS2)
modSSc8 <- glmer(Binary_death_total ~ Sed_level_stand_mg + Sed_exposure_days + (1 | Ref), 
              family = binomial, data = totalSS2)
modSSc9 <- glmer(Binary_death_total ~ Sed_level_stand_mg*Sed_exposure_days + (1 | Ref), 
              family = binomial, data = totalSS2)
modSSc10 <- glmer(Binary_death_total ~ Sed_level_stand_mg + (1 | Gsp) + (1 | Ref), 
              family = binomial, data = totalSS2)
...
#              family = binomial, data = totalSS2)
#Error in pwrssUpdate(pp, resp, tol = tolPwrss, GQmat = GQmat, compDev = compDev, : (maxstephalfit) PIRLS step-halvings failed to reduce deviance in pwrssUpdate
modSSc13 <- glmer(Binary_death_total ~ Sed_level_stand_mg + (1 | Ref_name/Ref), 
              family = binomial, data = totalSS2)
